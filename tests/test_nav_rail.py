...
язычки, анимация разворота — тестами не покрыт и покрыт быть не может
в этой форме. Разворот и его непереигрывание проверяются ТОЛЬКО
живьём (шаг 1 и шаг 8 протокола). Зелёный прогон этого файла не
означает, что AC-5 выполнен.
"""
