...
app.layout = dbc.Container([
    ...
    dbc.Row([
        ...
        ], width=9, style={"margin-left": "25%"})  # Отступ для sidebar

    ], className="g-0"),  # Убираем отступы между колонками
...
def display_page(pathname):
    ...
    if pathname is None or pathname == "/" or pathname == "/dashboard":
        ...
        return create_dashboard_layout(), create_page_header("Дашборд", "Обзор финансов")
        ...
        return html.Div([
            html.H2("Накопительные цели"),
            html.P("Здесь будут цели накоплений")
        ...
        ], className="text-center mt-5"), create_page_header("Ошибка", "Страница не найдена")
...
if __name__ == "__main__":
    ...
    app.run_server(
        debug=debug,
        port=port,
        host='0.0.0.0'  # Доступ извне (для Docker)
    )
