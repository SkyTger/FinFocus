...
1. **Полная верификация:**
   ```bash
   source .venv/bin/activate && black . && flake8 . && pytest
   ```
   Исправлять до полного прохождения.

2. **Коммит правок** (если были):
   ```bash
   git add . && git commit -m "chore: final QA fixes [protocol-0011/06]"
   ```
...
5. **Финальный коммит протокола:**
   ```bash
   git add .protocols/0011-chips-bulk-export/ && git commit -m "docs(protocol): finalize 0011-chips-bulk-export [protocol-0011/06]"
   git push
   ```
...
```
(Протокол 0011-chips-bulk-export — Финализация):
