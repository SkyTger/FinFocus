# Шаг 2: Создание DashboardService

## Briefing
- **Цель:** Создать новый сервис `DashboardService` для агрегации данных дашборда из CalendarService и GoalService.
- **Ключевые файлы:**
  - `app/services/dashboard_service.py` (создать)
- **Additional info:**
  - DashboardService использует composition (не наследование): создает экземпляры CalendarService и GoalService
  - Три публичных метода: `get_overview_metrics()`, `get_cashflow_data()`, `get_recent_transactions()`
  - User ID hardcoded = 1 (до реализации авторизации)
  - Для пустой БД возвращаются нули, не ошибки
  - Cashflow данные получаются одним SQL-запросом с GROUP BY (не N вызовов get_month_summary)

## Sub-tasks

1. **Создать файл** `app/services/dashboard_service.py` со следующим содержимым:
...
class DashboardService:
    ...
    def get_overview_metrics(
        ...
        if period == "month":
            ...
            summary = self._calendar_service.get_year_summary(user_id, reference_date.year)
            period_income = summary["total_income"]
            period_expense = summary["total_expense"]
...
1. **Выполнение:** Создай файл с указанным содержимым.
2. **Верификация:** После создания запусти проверки:
   ```bash
   cd /home/skytiger/PycharmProjects/worktrees/0003-dashboard-integration
   black app/services/dashboard_service.py
   flake8 app/services/dashboard_service.py
   ```
3. **Фиксация:** После успешной верификации:
   - Добавь запись в `log.md`
   - Обнови `context.md`: `Current Step` = 3
   - Проверь ветку main
4. **Коммит**: `git add . && git commit -m "feat(dashboard): add DashboardService for data aggregation [protocol-0003/02]"`. Push.
