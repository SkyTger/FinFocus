# Шаг 4: Unit Tests для OnboardingService

## Briefing

- **Цель:** Написать 8+ unit тестов для OnboardingService
- **Ключевые файлы:**
  - `tests/test_onboarding_service.py` — NEW
- **Доп. информация:** Покрытие: get_status, complete_with_balance, skip, edge cases

## Sub-tasks

1. **Создать тесты** (`tests/test_onboarding_service.py`):
   ```python
   ...
   class TestOnboardingServiceGetStatus:
       ...
       def test_new_user_first_launch_true(self, db_session):
           ...
           user = User(name="Test", starting_balance=Decimal("0"), first_launch=True)
           db_session.add(user)
           db_session.commit()
...
2. **Запустить тесты**:
   ```bash
   pytest tests/test_onboarding_service.py -v
   ```
...
3. Обнови `log.md` — количество тестов, покрытие
4. Обнови `context.md` — Current Step: 5, Next Action: Шаг 5
5. Коммит: `git add . && git commit -m "test(onboarding): add unit tests for OnboardingService [protocol-0014/04]"`
6. Push
7. Отчёт по формату
