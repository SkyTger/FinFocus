# Solution v4: `PanelData` за одну сессию, «вчера» из расширенного списка дат (платежи) и резерва дня `ref`, сайдбар без Output-колбэков
...
1. **«Вчера»: платежи считаются одним вызовом хелпера на расширенном списке дат (Подход A критика), резерв — значением дня `ref` (см. п. 2).** Критик прав: `_payments_tail_by_day` (`money_layers_service.py:423-462`) — **оконная**, а не точечная функция. Прочитано тело: `by_day` собирается по всему списку `payments`, но затем `result` наполняется проходом `for day in reversed(window_dates)`, и `tail` накапливается **из того же списка**. На `window_dates=[yday]` цикл делает одну итерацию, `tail` ещё `Decimal("0")`, `yday < payments_end` → `result[yday] = Decimal("0")`. Плюс подпись действительно `(payments, window_dates, payments_end)` — в v2 аргументы были переставлены. v3 передаёт **`calc_dates = [yday] + window_dates`** одним вызовом и берёт из результата и `yday`, и дни окна; `days` строится по-прежнему только из `window_dates`.
2. **`_goals_part_by_day` остаётся на `window_dates`; «вчера» берёт резерв дня `ref` (critique-v3, №2).** v3 передавала ему тот же `calc_dates` «для единственности списка» и обосновывала это тем, что от состава списка функция не зависит. Утверждение о независимости верно (перепроверено по телу `:578-597`: значение каждого ключа — функция только от `day` и `savings_by_date`), но из него не следует, что значение **для ключа `yday`** осмысленно: `month_start`/`month_end` берутся **по самому дню `D`** (`:580-581`), поэтому 1-го числа для ключа `yday` границы месяца — это границы **прошлого** месяца, из которого `savings_by_date` содержит ровно один день (`collect_start = min(_month_start(ref), yday)` = `yday`), а `committed(yday)` пуст, потому что `yday == _month_end(yday)`. Итог был бы `goals_part[yday] = max(0, monthly_budget − savings(yday))` — полный бюджет **текущей настройки**, отнесённый к прошлому месяцу; окошко «Вчера» показало бы завышенный резерв и заниженное «Свободно», посчитанные по другому месяцу, чем соседнее «Сегодня». Собственный докстринг функции (`:507-511`) называет `reference_date == 1-е число` вырожденным случаем — то есть нарушалась посылка функции, а не её тело. v4 применяет **предпочтительный вариант критика**: `_payments_tail_by_day` получает `calc_dates` (ключ `yday` ему нужен и корректен), `_goals_part_by_day` остаётся на `window_dates`, а `_yesterday_slice` берёт `goals_part[ref]` — «резерв, настроенный на начало периода». Расхождение «вчера»/«сегодня» по резерву становится **физически невозможным** (одно и то же число из одного словаря), а не просто согласованным.
...
Плюс: расхождение цифры «Аналитика» с месячным слоем «Платежи» объявлено (решение владельца), единая механика идемпотентности Store-фокусов, `_is_empty` через `savings_by_date` признан зависимым от `collect_start`, одна константа lookback, `dip_*` игнорируется при `EMPTY`.

**Что изменила v4 относительно v3** (правки по критике v3, вердикт MINOR_FIXES, блокеров нет):

- **`AllocationService` конструируется без аргументов** — `AllocationService().calculate_allocation(...)`. Перепроверено по телу `app/services/allocation_service.py`: `def __init__` в файле **отсутствует вообще** (класс объявлен на `:18`, первый метод — `calculate_allocation` на `:25`), сессию сервис не хранит и работает исключительно на переданном списке ORM-объектов. Все пять существующих вызовов в проекте — `AllocationService()` (`goals.py:1773`, `:2520`, `:3254`, `:3317`, `redistribution_service.py:38`; в тестах — ещё восемь, тем же способом). `AllocationService(self.session)` из v3 дал бы `TypeError` внутри блочного `except` → карточка Цели в `FAILED` на **любой** базе с целями, молча, с трейсбеком в лог. Строка «Зависимости» и стратегия загрузки поправлены: это единственный **stateless** сервис в списке, а не session-сервис. В шаг 4 добавлен тест `goals=OK` на фикстуре с двумя целями и непустым бюджетом — в наборе v3 такого теста не было (обе фикстуры AC-5 пустые, тест деградации ожидает `FAILED` как правильный результат, тесты карточек кормятся словарями).
- **`goals_part` для «вчера» берётся по дню `ref`, а не по `yday`** (см. п. 2 выше). `_goals_part_by_day` вызывается на `window_dates` — так же, как в куске 1.
- **Сессии посчитаны по пути сборки щитка, а не «за рендер дашборда»**: формулировка v3 «четвёртой не было» неверна. `grep get_db_session app/components/dashboard.py` даёт три попадания: `:871`, `:1002` и `:1191`; третье — внутри `toggle_balance_toast` (`:1169-1200`), который висит на `url.pathname` + `balance-alert-toast.is_open` + `profile-updated` с `prevent_initial_call=False` и открывает сессию под `OnboardingService.get_status` на каждой загрузке дашборда. Эта сессия остаётся вне scope по FR-4 намеренно, но в бюджет NFR-1 входит и названа отдельной строкой.
- Три уточнения формулировок: ветка ошибки `load_and_navigate_calendar` намеренно не пишет `focus_applied_ts`; контракт `_empty_*` для всех пяти блоков задан по полям; постоянно пустая подпись окошка «Вчера» вынесена в ручной чек-лист как вопрос к живой базе.

**Правка после критики v4** (1 🟡 + 3 🟢, блокеров нет — вносится в v4, отдельной версии не требует):
- **Перечень сессий строится командой, а не памятью** (critique-v4, №1). Прогон `grep -rn get_db_session app/components/ app/main.py` нашёл ещё две сессии на тех же путях, которых не было ни в одном из трёх предыдущих перечней: `onboarding_wizard.py:153` (`check_onboarding_and_validate` — Input `url.pathname`, `prevent_initial_call=False`, **guard'а по pathname нет**, поэтому сессия открывается на любом переходе, включая все разделы) и `sidebar.py:178` (`update_sidebar_profile` — сайдбар стоит **статически** в layout `main.py:58-61`, поэтому колбэк срабатывает и на дашборде). Итого до куска 2 на открытии `/dashboard` было **6** сессий, а не 4; после — **3**, из них 2 вне scope по FR-4. Побочный вывод, потерянный в v3: выигрыш Подхода B **больше** заявленного — `sidebar.py:178` с дашборда уходит совсем, а на разделах одна сессия `render_sidebar_slot` заменяет прежнюю. В план шага 12 внесена **команда**, а не число: три итерации перечень расширялся ровно настолько, насколько указывал критик, потому что метод оставался тем же.
- Заголовок секции расчёта «вчера» и фраза обзора приведены к факту: на расширенном списке считается **один** хелпер (платежи), резерв берётся значением дня `ref`.
- RTM #89 получил отсылку «по резерву — см. #93»; в контракте `_empty_analytics` сказано, что `month_label` строится из `reference_date`, а не из результата запроса (при `EMPTY` без пользователя блок до сервиса может не дойти).
...
| Компонент | Файл | Роль |
|---|---|---|
| `DashboardPanelService` | `app/services/panel_service.py` (новый, ~360 строк) | Read-only композитор: одна сессия, один `PanelData`. Знает `MoneyLayersService`, `GoalService`, `DashboardService`, `AnalyticsService`, `WishlistService` (все — session-сервисы, конструируются как `Service(self.session)`) + **`AllocationService` — stateless, конструируется `AllocationService()` без аргументов** (`__init__` у класса нет, `allocation_service.py:18-30`) + прямое чтение `User`. **Не знает** `CushionService`. О Dash не знает (C-2). |
...
| расширение модели слоёв | `app/schema/money_layers.py`, `app/services/money_layers_service.py` | ОДНО новое поле `yesterday: DayLayers \| None` + локальный `calc_dates` в `get_money_layers` (**только для `_payments_tail_by_day`**; `_goals_part_by_day` остаётся на `window_dates`, critique-v3 №2). `days`, `today`, `min_free`, `is_empty`, `window_is_flat` — форма не меняется. |
...
Открытие /dashboard
  ...
  │                           │                        window_dates, monthly_budget)  ← БЕЗ yday
  │                           │    days      = [_split_day(d) for d in window_dates]
  │                           │    yesterday = _split_day(...[ref-1]...)  ← из ТЕХ ЖЕ dict
  │                           ├─ try: _goals_block()      session.get(User) + Goal
  │                           │        + AllocationService() ← БЕЗ сессии (stateless)
...
### Расчёт «вчера»: платежи — из расширенного списка, резерв — из дня `ref` (блокер №1 v2, №2 v3)
...
# app/services/money_layers_service.py :: get_money_layers
    ...
    # Служебный список дат ДЛЯ ОДНОГО хелпера — НЕ окно модели
    # (critique-v2, блокер №1). days строится ТОЛЬКО из window_dates;
    # calc_dates нужен потому, что _payments_tail_by_day — ОКОННАЯ
    # функция: она накапливает суффиксную сумму проходом по ПЕРЕДАННОМУ
    # списку (:455-461), и на одноэлементном списке вернула бы
    # Decimal("0") для любого дня.
    #
    # ВТОРОМУ хелперу calc_dates НЕ передаётся (critique-v3, №2).
    # _goals_part_by_day от состава списка не зависит, но берёт границы
    # МЕСЯЦА по самому дню D (:580-581): 1-го числа для ключа yday это
    # границы ПРОШЛОГО месяца, из которого savings_by_date содержит
    # ровно один день, а committed пуст (yday == _month_end(yday)) —
    # то есть ключ был бы посчитан по чужому месяцу. Её собственный
    # докстринг (:507-511) называет этот случай вырожденным.
    yday = ref - LOOKBACK
    ...
    ...
    # window_dates, НЕ calc_dates (critique-v3, №2): вызов остаётся
    # ТОЧНО таким, как в куске 1 — этот хелпер вообще не правится и
    # не получает нового входа.
    goals_part = self._goals_part_by_day(savings_by_date, window_dates, monthly_budget)
    ...
    yesterday = self._yesterday_slice(
        balances, payments_tail, goals_part, cushion_threshold, yday, ref
    )
```
...
| `_goals_part_by_day` (тело `:578-597`, докстринг `:462-577`) | `for day in window_dates:` → `month_start = _month_start(day)`, `month_end = _month_end(day)` (`:580-581`) → полный обход `savings_by_date.items()` с фильтром `month_start <= op_date <= day` (consumed) / `day < op_date <= month_end` (committed); `result[day] = max(0, monthly_budget − consumed − committed)`. **Note-секция `:507-511`** прямым текстом: «Прошлые месяцы в окно не попадают… `month_start(D) >= collect_start` для всех D окна, КРОМЕ вырожденного случая `reference_date == 1-е число`» | **От состава списка не зависит** (перепроверено: значение каждого ключа — функция только от `day` и `savings_by_date`), **но посылка функции о диапазоне для `yday` нарушается** (critique-v3, №2). Границы берутся по самому `D`: 1-го числа для ключа `yday` это границы **прошлого** месяца; `savings_by_date` собран от `collect_start = min(_month_start(ref), yday) = yday`, то есть содержит из того месяца **ровно один день**; `committed(yday) = 0`, потому что `yday == _month_end(yday)`. Значение было бы `max(0, monthly_budget − savings(yday))` — полный бюджет **текущей настройки**, отнесённый к чужому месяцу. **Решение v4: `calc_dates` ему НЕ передаётся** — вызов остаётся `(savings_by_date, window_dates, monthly_budget)`, точно как в куске 1, а «вчера» берёт `goals_part[ref]` (см. `_yesterday_slice`). Ключа с нарушенной посылкой не возникает вовсе, докстринг `:507-511` остаётся верным буквально. |
...
сбора операций и служебным списком calc_dates (входом ОДНОГО
хелпера — _payments_tail_by_day), из которых считается отдельное
поле yesterday. _goals_part_by_day этой константы не видит вовсе:
резерв «вчера» берётся по дню reference_date (critique-v3, №2).
"""
```
...
```python
    def _yesterday_slice(
        ...
        Принимает УЖЕ ПОСЧИТАННЫЕ payments_tail и goals_part и НЕ
        вызывает хелперы повторно. Но берёт из них РАЗНЫЕ ключи, и
        это не небрежность, а следствие разных контрактов хелперов:

          payments_tail[yday]  — своим ключом. Хелпер считался на
            calc_dates = [yday] + window_dates, ключ yday в нём есть
            и корректен: суффиксная сумма (yday, payments_end] есть
            осмысленное «сколько платежей ещё уйдёт», и она НЕ зависит
            от того, в каком месяце лежит yday.
          goals_part[ref]      — ключом СЕГОДНЯШНЕГО дня, потому что
            ключа yday в словаре нет вовсе: _goals_part_by_day
            вызывается на window_dates.

        ПОЧЕМУ РЕЗЕРВ БЕРЁТСЯ ПО ref (critique-v3, №2):
        _goals_part_by_day берёт границы МЕСЯЦА по самому дню D
        (:580-581). Для yday 1-го числа это границы ПРОШЛОГО месяца,
        из которого savings_by_date содержит ровно один день
        (collect_start = min(_month_start(ref), yday) = yday), а
        committed(yday) = 0, потому что yday == _month_end(yday).
        Значение было бы max(0, monthly_budget - savings(yday)) —
        полный бюджет ТЕКУЩЕЙ настройки, отнесённый к чужому месяцу:
        завышенный резерв и заниженное «Свободно» у «Вчера», причём
        посчитанные по другому месяцу, чем соседнее «Сегодня». Ровно
        то расхождение соседних чисел, от которого лечит FR-6.
        Докстринг самой функции (:507-511) называет reference_date ==
        1-е число вырожденным случаем — то есть нарушалась её посылка,
        а не её тело.

        goals_part[ref] — это «резерв целей, настроенный на начало
        периода»: бюджет минус то, что уже ушло и ещё уйдёт в ТЕКУЩЕМ
        месяце. Он определён для ЛЮБОГО дня месяца (границы месяца
        всегда месяц ref, а savings_by_date покрывает весь этот месяц
        по построению collect_start), поэтому ветвления по датам нет
        и вырожденного случая нет. Побочно: расхождение резерва между
        «вчера» и «сегодня» становится ФИЗИЧЕСКИ невозможным — это
        одно и то же число из одного словаря, что для FR-6 строже,
        чем «согласовано».

        Семантика поля названа прямо: reserve_configured «вчера» —
        это настройка периода, а не реконструкция прошлого. Истории
        бюджета в схеме нет (C-4, users.monthly_savings_budget —
        одна настройка на все месяцы, database.py:99), поэтому
        реконструировать прошлый месяц было бы нечем — см. Note
        «ДОПУЩЕНИЕ „БЮДЖЕТ НЕ МЕНЯЛСЯ ВНУТРИ МЕСЯЦА"» в
        _goals_part_by_day.

        ПОЧЕМУ payments НЕ ТОЧЕЧНЫМ ВЫЗОВОМ (critique-v2, блокер №1):
        ...
        Считается ТЕМ ЖЕ _split_day, что дни окна: расхождение
        способа счёта между «вчера» и «сегодня» было бы новым
        источником противоречащих цифр (FR-6).
        ...
        # goals_part[ref], НЕ [yday]: ключа yday в словаре нет
        # (хелпер считался на window_dates), и он был бы вырожден
        # 1-го числа — critique-v3, №2.
        reserve_configured = cushion_threshold + goals_part.get(ref, Decimal("0"))
...
| `_goals_part_by_day` (вызов) | `(savings_by_date, window_dates, monthly_budget)` | **без изменений** — `(savings_by_date, window_dates, monthly_budget)` | **(v4, critique-v3 №2)** `calc_dates` ему НЕ передаётся. Тело берёт границы месяца по самому дню `D` (`:580-581`), поэтому ключ `yday` 1-го числа считался бы по **прошлому** месяцу, из которого `savings_by_date` содержит один день, а `committed` пуст → полный бюджет текущей настройки, отнесённый к чужому месяцу. Посылка функции о диапазоне (`Note :507-511`, «вырожденный случай `reference_date == 1-е число`») нарушалась бы. Ключа с нарушенной посылкой не возникает: «вчера» берёт `goals_part[ref]` |
| `_yesterday_slice.reserve_configured` (**новое**) | — | `cushion_threshold + goals_part[ref]` | **(v4)** «резерв, настроенный на начало периода». Определён во **все** дни месяца: границы `goals_part[ref]` — всегда месяц `ref`, а `savings_by_date` покрывает весь этот месяц по построению `collect_start = min(_month_start(ref), yday)`. Ветвления по датам нет, вырожденного случая нет. Расхождение резерва «вчера»/«сегодня» физически невозможно (одно число из одного словаря) — FR-6 строже, чем «согласовано». Цена названа: `reserve_configured` «вчера» — это **настройка периода**, а не реконструкция прошлого месяца; реконструировать было бы нечем (истории бюджета в схеме нет, C-4) |
...
class DashboardPanelService:
    ...
    СТРАТЕГИЯ ЗАГРУЗКИ (FR-6, NFR-1) — по фактическим вызовам:
        ...
        Сессий НА ПУТИ СБОРКИ ЩИТКА: 1 (было 3, critique-v2 №8;
        формулировка уточнена по critique-v3 №3 — «за рендер
        дашборда» было неверно, см. следующий абзац).
          ...
          dashboard.py:687, а не из колбэка).
        ...
        Сессия онбординг-тоста — ВНЕ SCOPE, но в бюджете NFR-1
        (critique-v3, №3). grep get_db_session app/components/
        dashboard.py даёт ТРИ попадания: :871, :1002 и :1191.
        Третье — внутри toggle_balance_toast (:1169-1200): колбэк
        висит на Input url.pathname + balance-alert-toast.is_open +
        profile-updated с prevent_initial_call=False и на КАЖДОЙ
        загрузке /dashboard открывает сессию под
        OnboardingService.get_status(DEFAULT_USER_ID). Она
        СОХРАНЯЕТСЯ НАМЕРЕННО: FR-4 требует онбординг-тост не
        трогать, а его логика к щитку отношения не имеет. Но в
        стоимость открытия страницы она входит, поэтому названа
        здесь явно — чтобы цифра «1» не поехала в knowledge-bank
        как «одна сессия на открытие /dashboard».

        ПЕРЕЧЕНЬ СТРОИТСЯ КОМАНДОЙ, А НЕ ПАМЯТЬЮ (critique-v4, №1).
        Три итерации подряд перечень был неполон, потому что grep шёл
        по одному файлу, о котором шла речь (v2: «1» → 3; v3: «3,
        четвёртой не было» → 4; v4: «4» → 6). Правильный способ:
            grep -rn get_db_session app/components/ app/main.py
        и разбор КАЖДОГО попадания по признаку «срабатывает ли этот
        колбэк на данном пути». Ниже — результат такого прогона;
        при правках шага 12 команду выполнить заново, а не доверять
        этому списку.

        Сессии на пути ОТКРЫТИЯ /dashboard:
          1. dashboard.py:1002  — сборка щитка (_load_dashboard_components).
                                  ЭТО и есть «1 вместо 3» куска 2.
          2. dashboard.py:1191  — онбординг-тост (toggle_balance_toast),
                                  Input url.pathname, prevent_initial_call=False.
                                  ВНЕ SCOPE по FR-4, сохраняется намеренно.
          3. onboarding_wizard.py:153 — check_onboarding_and_validate,
                                  Input url.pathname, prevent_initial_call=False,
                                  guard'а по pathname НЕТ: при
                                  triggered_id == "url" сессия открывается
                                  на ЛЮБОМ переходе, включая все разделы.
                                  ВНЕ SCOPE (онбординг, FR-4).
          0. сайдбар            — render_sidebar_slot возвращает []
                                  ДО открытия сессии.
          Итого после куска 2: 3 (из них 2 — вне scope).

        Сессии на пути ПЕРЕХОДА на /calendar|/goals|/transactions|/analytics:
          1. main.py (render_sidebar_slot) — профиль сайдбара, цена Подхода B.
          2. onboarding_wizard.py:153     — тот же визард, вне scope.
          + сессии самого раздела (куском 2 не меняются).

        Сессии на пути ПРАВКИ ПРОФИЛЯ (profile-updated):
          1. main.py (render_sidebar_slot) + 2. dashboard.py:1191 (тост).

        БЫЛО до куска 2 на открытии /dashboard — 6, а не 4:
          dashboard.py:1002, dashboard.py:871 (_build_cushion_card_readonly),
          wishlist.py:33 (build_wishlist_widget из layout),
          dashboard.py:1191 (тост), onboarding_wizard.py:153 (визард),
          sidebar.py:178 (update_sidebar_profile — сайдбар стоит
            СТАТИЧЕСКИ в layout, main.py:58-61, поэтому колбэк
            срабатывает и на дашборде).
        СТАЛО: 3. Выигрыш БОЛЬШЕ заявленного в v3/v4: sidebar.py:178
        не просто переезжает, а с дашборда УХОДИТ совсем (там сайдбара
        нет), а на разделах заменяется одной сессией render_sidebar_slot
        вместо двух (update_sidebar_profile + отсутствовавшее чтение
        для подсветки). Это дополнительный довод в пользу Подхода B,
        потерянный в v3.
        ...
        Блоки карточек, добавляемые куском 2:
          ...
          _goals_block     — session.get(User, uid) [identity map: User уже
                           ...
                           + AllocationService().calculate_allocation
                             (0 запросов; сервис STATELESS — __init__
                             у класса НЕТ, allocation_service.py:18-30,
                             сессию не хранит и работает на переданном
                             списке ORM Goal. Конструируется БЕЗ
                             аргументов, как во всех пяти
                             существующих вызовах проекта:
                             goals.py:1773/2520/3254/3317,
                             redistribution_service.py:38.
                             AllocationService(self.session) дал бы
                             TypeError — critique-v3, №1)
...
**Контракт `_empty_*` (critique-v3, №5).** Пустое состояние — это `CardStatus.EMPTY` **плюс нейтральные значения не-Optional полей**, а не «что попало под типом». Правило одно на все пять блоков: **Optional-поля → `None`; числовые не-Optional → `0` / `Decimal("0")` / `0.0`; строковые не-Optional → `""`; списки → `[]`; bool → `False`; `href`-поля заполняются как обычно** (дверь работает и на пустой карточке — FR-2 «находимость разделов» не зависит от наличия данных; `href` — константа маршрута, не число). `""` вместо осмысленного текста выбран сознательно: карточка при `EMPTY` рисует **свой** текст пустого состояния из build-функции, а не подставляет строку из данных, — иначе получилось бы два источника формулировки. Ни одно из этих значений в дерево при `EMPTY` не попадает (`status` — единственный источник правды отрисовки, #81), поэтому нулей и `₽` в дереве нет; но зафиксировать их нужно, чтобы реализация не положила туда `"—"`, `"0 ₽"` или `"Нет целей"`, которые при будущей правке build-функции протекли бы в UI как числовой артефакт (AC-5).
...
def _empty_goals() -> GoalsCardData:
    ...
    return GoalsCardData(
        ...
        top_goal_id=None, top_goal_name=None, top_goal_progress=0.0,
        top_goal_current=Decimal("0"), top_goal_target=Decimal("0"),
        top_goal_target_date=None, top_goal_href=None,
        others_count=0, others_behind_count=0, others_summary="",
        cushion_is_configured=False, cushion_progress=0.0, cushion_label="",
    )
```

Остальные четыре `_empty_*` — по тому же правилу, полями своих контрактов:

| Функция | Не-Optional поля при `EMPTY` | Optional → `None` |
|---|---|---|
| `_empty_calendar()` | `days=[]` (**не** три окошка с нулями: карточка при `EMPTY` рисует своё пустое состояние), `dip_is_strong=False` | `dip_date`, `dip_free`, `dip_href`. **Плюс** оговорка #91: `dip_*` не рисуются при `status != OK`, даже если непустые — то есть даже если сюда однажды положат значения, UI их не покажет |
| `_empty_operations()` | `recent=[]`, `upcoming=[]`, `recent_href` / `upcoming_href` — **заполняются как обычно** (`href` — маршрут, не число; дверь работает и на пустой карточке) | — (Optional-полей нет) |
| `_empty_analytics()` | `month_label` — **заполняется как обычно** («августа»: это подпись периода, не цифра, и она нужна пустому состоянию тоже). **Формируется из `reference_date`, а не из результата запроса** (critique-v4, №4): при `EMPTY` из-за отсутствующего пользователя блок до `AnalyticsService` может не дойти, поэтому подпись строится из даты отсчёта и доступна всегда. `month_total=Decimal("0")`, `top_category_total=Decimal("0")`, `top_category_share=0.0`, `structure=[]`, `href` как обычно | `top_category_name` |
| `_empty_wishlist()` | `items=[]`, `total_count=0` | — (Optional-полей нет) |

**Тест на этот контракт** (шаг 4): на фикстуре пустой базы для каждого из пяти срезов проверяется не только `status == EMPTY`, но и что **все не-Optional поля равны перечисленным нейтральным значениям** — иначе `_empty_*` можно незаметно заполнить текстом, а тест AC-5 «нет `₽`/`%`» это не поймает (он смотрит дерево, а не данные).
...
```python
    def _goals_block(self, user_id: int, layers: MoneyLayersData) -> GoalsCardData:
        ...
        # AllocationService — БЕЗ АРГУМЕНТОВ (critique-v3, №1).
        # У класса НЕТ __init__ (allocation_service.py:18-30): он
        # stateless, сессию не хранит и работает исключительно на
        # переданном списке ORM Goal. Все пять существующих вызовов
        # в проекте — AllocationService() (goals.py:1773/2520/3254/3317,
        # redistribution_service.py:38). AllocationService(self.session)
        # дал бы TypeError: AllocationService() takes no arguments —
        # внутри блочного except это молча превратило бы карточку Цели
        # в FAILED на ЛЮБОЙ базе с целями.
        # Stateless НЕ значит «можно вызывать вне сессии»: goals —
        # живые ORM-объекты, и calculate_allocation читает
        # goal.priority/.status/.monthly_contribution/.id/.name,
        # поэтому вызов обязан быть внутри открытой сессии. Для
        # _goals_block это и так выполнено.
...
- `_goals_part_by_day(savings_by_date, **window_dates**, monthly_budget)` — вызов **не меняется** (critique-v3, №2): `calc_dates` этому хелперу не передаётся, потому что он берёт границы месяца по самому дню `D` (`:580-581`) и ключ `yday` 1-го числа посчитался бы по прошлому месяцу;
- новый `_yesterday_slice(balances, payments_tail, goals_part, cushion_threshold, yday, **ref**)` — `payments_tail[yday]` своим ключом, `goals_part[**ref**]` ключом сегодняшнего дня («резерв, настроенный на начало периода»);
...
8b. **`yesterday["reserve_configured"]` НА ЗНАЧЕНИЕ** (целевой тест critique-v3 №2, симметричен п. 1 для `payments`). Фикстура: `ref` = **1-е число** + `monthly_savings_budget` > 0 + savings-операции **в прошлом месяце** (в том числе в его последний день = `yday`) + savings-операция в текущем месяце. Ассерты: (а) `data["yesterday"]["reserve_configured"] == data["days"][0]["reserve_configured"]` — **резерв «вчера» равен резерву «сегодня» по построению**, расхождение физически невозможно; (б) числовое равенство `cushion_threshold + max(0, monthly_budget − consumed(ref) − committed(ref))`, где `consumed`/`committed` считаются **по текущему месяцу** — то есть операции прошлого месяца в резерв «вчера» **не входят**; (в) **mutation-проверка**: локально заменить `goals_part.get(ref, …)` на `goals_part.get(yday, …)` **и** передать `calc_dates` в `_goals_part_by_day` — тест обязан покраснеть (без второй части словарь не содержит ключа `yday` и `.get` вернёт дефолт `0`, что тоже покраснеет, но по другой причине; проверить надо обе). Плюс тот же ассерт (а) на фикстуре с `ref` **в середине месяца** — вариант корректен не только 1-го числа;
...
- `_goals_block(user_id, layers)` — **`session.get(User, uid)` → `None` = `EMPTY`**; три поля (`cushion_target`, `monthly_savings_budget`, `savings_mode`) одним чтением; `GoalService(self.session).get_all_by_user(ACTIVE)` → **`AllocationService().calculate_allocation(goals, monthly_budget, savings_mode)` — конструктор БЕЗ аргументов** (critique-v3 №1: `def __init__` у класса нет, `allocation_service.py:18-30`; подпись метода `:25-30`; все пять вызовов проекта — `AllocationService()`); `others_behind_count` из `allocation["results"]` по `shortfall > 0`; подушка из `layers["cushion_threshold"]` + `layers["today"]["balance"]` + `User.cushion_target`, **без `CushionService.get_settings`**; пять `_empty_*` по заданному контракту (`""` и нули, не текст);
...
- **`goals=OK` на фикстуре С ЦЕЛЯМИ (целевой тест critique-v3 №1)**: `User` с `monthly_savings_budget = 30 000` и `savings_mode="free"`, **две** активные цели с `priority=1/2` и `monthly_contribution` так, чтобы бюджета хватило первой и не хватило второй. Ассерты: `panel["goals"]["status"] == CardStatus.OK`; `others_count == 1`; **`others_behind_count == 1`** (разбор `allocation["results"]` по `shortfall > 0`); `top_goal_name` — цель с `priority=1`; `top_goal_current`/`top_goal_target` — `Decimal`. **Зачем именно этот тест:** в наборе v3 не было ни одного теста, обязанного покраснеть от `AllocationService(self.session)` — обе фикстуры AC-5 пустые (до `AllocationService` дело не доходит), тест деградации ожидает `FAILED` как **правильный** результат, а тесты карточек кормятся словарями. `TypeError` из конструктора поглотился бы блочным `except` и дал бы `FAILED` молча. Этот тест краснеет и на неверном конструкторе, и на опечатке в разборе `allocation["results"]`. Плюс ассерт «в логах нет трейсбека по `goals`» — чтобы `FAILED` нельзя было принять за штатный путь;
- **контракт `_empty_*` на уровне ДАННЫХ (critique-v3 №5)**: на фикстуре пустой базы для каждого из пяти срезов — все не-Optional поля равны заданным нейтральным значениям (`others_summary == ""`, `cushion_label == ""`, `top_goal_progress == 0.0`, `cushion_progress == 0.0`, `month_total == Decimal("0")`, `structure == []`, `total_count == 0`), Optional — `None`, `href`-поля непусты. Тест AC-5 «нет `₽`/`%`» смотрит дерево и этого не поймает;
...
**Шаг 9. Приёмники контекста (FR-3) с владением `url.search` и единой идемпотентностью.** `main.py`: `handle_panel_query_params` + `_OWNED_SEARCH_PATHS` + три Store'а (`open-wishlist-trigger`, `calendar-focus-date`, `goals-focus-goal`); payload новых Store'ов — `{"value":…, "ts":…}`. `calendar.py`: `Input("calendar-focus-date")` шестым Input'ом в `load_and_navigate_calendar`; **двойной guard идемпотентности** (`ctx.triggered_id` == сам Store **И** `ts != state.get("focus_applied_ts")`); ключи `focus_date`/`focus_applied_ts` в `calendar-state` (словарь уже возвращается, `:926-930`); **ветка `except` (`:938-948`) НЕ ПРАВИТСЯ — `focus_applied_ts` в неё намеренно не дописывается** (critique-v3, №4: ветка возвращает исходный `state` четвёртым элементом, `:944`; если фокус пришёл, а загрузка данных упала, `ts` не сохранится — и переприменения всё равно не будет, потому что при следующем срабатывании по любому другому Input'у **первое** условие guard'а `ctx.triggered_id == Store` уже не выполнится. Дописывать `ts` в ветку ошибки не нужно и вредно: это добавило бы второе место записи одного факта, а после падения загрузки пользователь как раз ДОЛЖЕН получить фокус при повторном клике); класс `calendar-day-focused` в `build_day_cell` (`css_classes` — список, `:596`); стиль в `calendar.css`. `goals.py`: якорные id в `_build_goal_card` (`:618`), узел `goals-focus-anchor`, колбэк `apply_goal_focus` с **той же** механикой. `transactions.py` и `analytics.py` — без правок.
...
**Шаг 12. Замер NFR-1 (время + запросы) и статика.** Замер как в куске 1 (`time.perf_counter` вокруг `_load_dashboard_components`) **плюс счётчик SQL-запросов** (`sqlalchemy.event` на `before_cursor_execute`); **плюс отдельный замер `render_sidebar_slot("/calendar", None)`** — цена Подхода B в числах, в протокол.

**Перечень сессий строится КОМАНДОЙ (critique-v4, №1).** Перед замером выполнить `grep -rn get_db_session app/components/ app/main.py` и разобрать каждое попадание по признаку «срабатывает ли этот колбэк на измеряемом пути» — три итерации подряд перечень был неполон именно потому, что grep шёл по одному файлу. Ожидаемый результат: открытие `/dashboard` — 3 сессии (щиток `:1002` + тост `:1191` + визард `onboarding_wizard.py:153`, две последние вне scope по FR-4); переход на раздел — сайдбар + визард + сессии раздела; до куска 2 на открытии `/dashboard` было 6.

**Что именно мерится и что в замер НЕ входит (critique-v3, №3).** Мерится **путь сборки щитка**: `_load_dashboard_components` (1 сессия, 4-9 коротких запросов + модель слоёв). В эту цифру **не входит** `toggle_balance_toast` (`dashboard.py:1169-1200`, сессия `:1191`), который срабатывает на той же загрузке `/dashboard` по `url.pathname` с `prevent_initial_call=False` и добавляет **1 сессию + 1 `SELECT User`** (`OnboardingService.get_status`). Он вне scope по FR-4 и куском 2 не трогается. Поэтому в протокол пишутся **три** числа, а не одно: (1) `_load_dashboard_components` — время и запросы; (2) «+1 сессия и +1 SELECT: онбординг-тост, вне scope, существует и до куска 2»; (3) `render_sidebar_slot("/calendar", None)` — цена Подхода B на переходе. Без разделения цифра в протоколе оказалась бы меньше реальной стоимости открытия страницы, а в knowledge-bank поехала бы как «одна сессия на `/dashboard`».

`black`, `flake8`, полный `pytest`.
...
- Внутренние, **session-сервисы** (конструируются `Service(self.session)`): `panel_service` → `MoneyLayersService`, `GoalService`, `DashboardService`, `AnalyticsService`, `WishlistService` + прямое чтение `User` (`app.models.database`).
- Внутренние, **stateless** (конструируется **без аргументов**): `AllocationService` (critique-v3, №1). Проверено по телу `app/services/allocation_service.py`: `def __init__` в файле **отсутствует**, класс объявлен на `:18`, первый метод — `calculate_allocation(self, goals, monthly_budget, savings_mode="free")` на `:25-30`; сессию сервис не хранит и работает исключительно на переданном списке ORM `Goal`. Все пять существующих вызовов проекта — `AllocationService()` (`goals.py:1773`, `:2520`, `:3254`, `:3317`, `redistribution_service.py:38`). `AllocationService(self.session)` дал бы `TypeError: AllocationService() takes no arguments`. **Stateless ≠ вне сессии**: метод читает `goal.priority`, `.status`, `.monthly_contribution`, `.id`, `.name` у живых ORM-объектов, поэтому вызов обязан быть внутри открытой сессии — в `_goals_block` это выполнено.
- `CushionService` из зависимостей композитора **исключён**; `GoalService.get_savings_budget`/`get_savings_mode` **не вызываются**. Все существуют, ни один не меняется (C-3).
...
| **R19** | **Четвёртый случай «оконный хелпер применён вне своего диапазона»** в куске 3 или при карточках на других страницах. Три уже были: `_forecast_balances` (критика v1), `_payments_tail_by_day` (критика v2), `_goals_part_by_day` (критика v3 №2) — и третий показал, что вопрос «зависит ли результат от состава списка» **недостаточен**. | Средняя (файл будет открыт снова) | Урок в `knowledge-bank/patterns/` в **расширенной** формулировке (заметка критика v3): у оконного хелпера **два** независимых вопроса, и оба обязательны. (1) **Зависит ли результат от состава списка?** `_payments_tail_by_day` — да (суффиксная сумма по переданному списку); `_goals_part_by_day` — нет. (2) **Для каких дат выполняются посылки функции?** `_goals_part_by_day` от состава честно не зависит и **всё равно** даёт для нового ключа число, посчитанное по чужому месяцу, потому что берёт границы месяца по самому дню — и её собственный докстринг предупреждает об этом прямым текстом в Note `:507-511`. Приём проверки, записываемый в урок: **прежде чем добавить дату в список оконного хелпера, прочитать его Note-секции на предмет допущений о диапазоне** — тело может быть корректно, а неверна применимость. Плюс: в докстринги `_payments_tail_by_day` и `_goals_part_by_day` — по строке с ответом на оба вопроса. Ловить четвёртый случай на ревью реализации дороже, чем записать правило. |
| **R20** | **`AllocationService(session)` или другой session-подобный вызов stateless-сервиса** — `TypeError` внутри блочного `except` → карточка `FAILED` **молча**, на любой базе с целями (critique-v3, №1). Профиль дефекта тот же, что у R3b: тесты пустых фикстур не доходят до вызова, тест деградации ожидает `FAILED` как правильный результат, тесты карточек кормятся словарями. | **Была высокая в v3 (дефект был в решении); в v4 закрыта** | Вызов исправлен на `AllocationService()`; в «Зависимости» сервис вынесен в отдельную категорию «stateless» с указанием, что `__init__` у класса нет; **целевой тест шага 4** `goals=OK` на фикстуре с двумя целями и непустым бюджетом с ассертом на `others_behind_count` — он и есть тест, обязанный покраснеть от неверного конструктора; плюс ассерт «в логах нет трейсбека по `goals`». Правило в `patterns/`: **прежде чем передать сессию в конструктор сервиса, проверить, что `__init__` у него есть** — в проекте это единственный сервис без него. |
...
| 62 | Эскиз v3: подпись окошка «2 операции» / «план» | эскиз | `operations_note` из `layers["upcoming_payments"]` (без запроса) для «сегодня»/«завтра». У «вчера» подпись **пустая ВСЕГДА**, а не «когда операций не было»: `payments` фильтруется `day >= reference_date` (`:404`), то есть вчерашних операций в модели нет по построению — `operations_note = None` для «вчера» на любой базе. **(v4, critique-v3 №6)** Это значит, что пользователь, глядя на три окошка, может прочитать постоянную пустоту как ложное утверждение «вчера операций не было». Эскиз ставил там `&nbsp;`, то есть формально по эскизу; но решение принимается **по живой базе**, а не по эскизу: пункт ручного чек-листа (проверка 18) — открыть дашборд на базе, где **вчера операции были**, и посмотреть, читается ли пустое место как «их не было». Если да — **место под подпись у окошка «вчера» убирается совсем** (`.pnl-day-note` не рендерится при `operations_note is None`, вместо `&nbsp;`-распорки), а вертикальный ритм трёх окошек держится `min-height` контейнера, а не пустой строкой. Развилка зафиксирована здесь, чтобы решение не приняли молча в CSS | Visual |
...
| 85 | **(v4, дополнена) Явные границы расчёта «вчера»** | FR-1.a, AC-3, FR-6 | Таблица границ: `_forecast_balances(ref-LOOKBACK, …)`, `collect_start = min(_month_start(ref), ref-LOOKBACK)`, **`calc_dates` как вход ОДНОГО хелпера — `_payments_tail_by_day`**; **`_goals_part_by_day` остаётся на `window_dates`, резерв «вчера» = `cushion_threshold + goals_part[ref]`** (#93); `None` → прочерк вместо «0 ₽»; строка про `_is_empty`/`savings_by_date` (#89, R5b) | Edge |
...
| 89 | **(v4, уточнена) «Вчера» считается тем же оконным каскадом, что окно — по платежам** (по резерву — см. #93) | FR-1.a, FR-6, C-3, C-5 | `calc_dates = [ref-LOOKBACK] + window_dates` → один вызов `_payments_tail_by_day(payments, calc_dates, payments_end)` (**правильный порядок аргументов**, подпись `:423-428`). `_goals_part_by_day` **на `window_dates`, вызов не меняется** (см. #93). **Тест на значение** `yesterday["payments"] == days[0]["payments"] + Σ payments(date == ref)` + **mutation-проверка**; в плане записано, что **тест инварианта суммы этот класс дефектов не ловит** (`_split_day` сохраняет сумму при любом входе) | Edge |
...
| 93 | **НОВОЕ (v4). Резерв «вчера» берётся по дню `ref`, а не по `yday`** | FR-1.a, FR-6, C-3, C-5 | `_goals_part_by_day` вызывается на `window_dates` (**вызов куска 1 не меняется**), `_yesterday_slice` берёт `goals_part[ref]`. Причина из тела: границы месяца берутся по самому дню `D` (`:580-581`), поэтому 1-го числа ключ `yday` считался бы по **прошлому** месяцу, из которого `savings_by_date` содержит один день (`collect_start = yday`), а `committed(yday) = 0` (`yday == _month_end(yday)`) → полный бюджет текущей настройки, отнесённый к чужому месяцу; докстринг функции сам называет этот случай вырожденным (`Note :507-511`). Следствие выбора: расхождение резерва «вчера»/«сегодня» **физически невозможно**, ветвления по датам нет, посылка хелпера не нарушается ни в один день. Цена названа: `reserve_configured` «вчера» — настройка периода, не реконструкция прошлого месяца (истории бюджета в схеме нет, C-4). **Тест на значение** `yesterday["reserve_configured"]` на фикстуре «`ref` = 1-е число + savings прошлого месяца» + равенство резерву `days[0]` + тот же ассерт в середине месяца + mutation-проверка (шаг 1, п. 8b) | Edge |
| 94 | **НОВОЕ (v4). `AllocationService` — stateless, конструируется без аргументов** | C-3, NFR-2, AC-4 | `def __init__` у класса **отсутствует** (`allocation_service.py:18-30`); сессию не хранит, работает на переданном списке ORM `Goal`; все пять вызовов проекта — `AllocationService()` (`goals.py:1773/2520/3254/3317`, `redistribution_service.py:38`). `AllocationService(self.session)` дал бы `TypeError` внутри блочного `except` → карточка Цели в `FAILED` **молча** на любой базе с целями. Исправлено в `_goals_block`, в «Зависимости» (отдельная категория), в стратегии загрузки. **Целевой тест** `goals=OK` на фикстуре с двумя целями и непустым бюджетом с ассертом `others_behind_count == 1` (шаг 4) — в наборе v3 такого теста не было; плюс ассерт «в логах нет трейсбека по `goals`». Риск R20 | Edge |
| 95 | **(v4-fix) Полный перечень сессий на путях — строится КОМАНДОЙ, не памятью** | FR-4, NFR-1, C-3 | Три итерации перечень был неполон, потому что `grep` шёл по одному файлу (v2: «1» → 3; v3: «3, четвёртой не было» → 4; v4: «4» → 6). Правильный способ, внесённый в план шага 12: **`grep -rn get_db_session app/components/ app/main.py`** + разбор каждого попадания по признаку «срабатывает ли колбэк на данном пути». Результат: открытие `/dashboard` **до** куска 2 — 6 сессий (`dashboard.py:1002`, `:871`, `wishlist.py:33`, `:1191` тост, `onboarding_wizard.py:153` визард, `sidebar.py:178` профиль сайдбара — сайдбар статичен в layout `main.py:58-61`, колбэк срабатывает и на дашборде); **после** — 3, из них 2 вне scope по FR-4 (тост `:1191`, визард `:153`, у которого **нет** guard'а по pathname: `prevent_initial_call=False` + `triggered_id == "url"` → сессия на любом переходе). Побочный вывод: выигрыш Подхода B **больше** заявленного — `sidebar.py:178` с дашборда уходит совсем, а на разделах одна сессия заменяет прежнюю. Замер шага 12 мерит только композитор — сказано явно; в протокол пишутся числа по путям, а не одно | Perf |
| 96 | **НОВОЕ (v4). Контракт `_empty_*` для всех пяти блоков** | AC-5, FR-5, NFR-2 | Правило: Optional → `None`; числовые не-Optional → `0`/`Decimal("0")`/`0.0`; строковые не-Optional → `""` (**не** «Нет целей», **не** «0%»); списки → `[]`; `bool` → `False`; `href` и `month_label` — как обычно (маршрут и подпись периода, не цифры). Текст пустого состояния рисует build-функция по `status`, а не данные — иначе два источника формулировки, а строка вида «0 ₽» в данных однажды протекла бы в дерево. Докстринг `_empty_goals` с перечислением всех полей + таблица по остальным четырём + **тест на уровне данных** (шаг 4): тест AC-5 «нет `₽`/`%`» смотрит дерево и этого не поймает | Edge |
| 97 | **НОВОЕ (v4). Ветка ошибки `load_and_navigate_calendar` намеренно не пишет `focus_applied_ts`** | FR-3, AC-2, NFR-2 | `except Exception` (`:938-948`) возвращает исходный `state` четвёртым элементом (`:944`) — правится **не будет**. Поведение безопасное: `ts` не сохранён, но переприменения не произойдёт, потому что при следующем срабатывании по любому другому Input'у **первое** условие guard'а (`ctx.triggered_id == Store`) уже не выполнится. Дописывать `ts` в ветку ошибки вредно: второе место записи одного факта + после сбоя загрузки повторный клик как раз **должен** сработать. Зафиксировано в шаге 9, чтобы при реализации не возникло соблазна | Integration |
...
| `app/services/money_layers_service.py` | **(v4)** только: `calc_dates` в `get_money_layers`, левая граница `_forecast_balances`, `collect_start` в `_horizons`, **точка вызова `_payments_tail_by_day`** (`window_dates` → `calc_dates`), новый `_yesterday_slice(..., yday, ref)`, заполнение поля, `Note` в докстринге `_is_empty`, **по строке в докстринги `_payments_tail_by_day` и `_goals_part_by_day`** об ответах на два вопроса R19 (зависимость от состава списка; для каких дат выполняются посылки). **Вызов `_goals_part_by_day` НЕ меняется** (`window_dates`, critique-v3 №2) — этот хелпер куском 2 не затрагивается вообще. **Тела** `_payments_tail_by_day`, `_goals_part_by_day`, `_split_day`, `_today_slice`, `_window_min_free`, `_is_empty`, `_user_data_markers`, `_goal_milestones`, `window_dates`, `days` — **без правок** | C-5, AC-3, блокер №1 v2, №2 v3 |
...
- `.obsidian-docs/knowledge-bank/modules/services.md` — `DashboardPanelService` (контракт материализации, преобразования типов, контракт `_empty_*`, стратегия загрузки: **«сессий на пути сборки щитка: 1 (было 3)» + отдельная строка «онбординг-тост `dashboard.py:1191` — ещё 1 сессия и 1 SELECT на загрузку `/dashboard`, вне scope по FR-4, куском 2 не трогается» + сессия сайдбара**; формулировка «за рендер дашборда» в knowledge-bank **не используется** — она и вводила в заблуждение); **`AllocationService` — единственный stateless-сервис проекта: `__init__` нет, конструируется `AllocationService()`, вызывать внутри открытой сессии (ORM-объекты)**; эволюция `MoneyLayersService` (поле `yesterday`, `calc_dates` как вход **одного** хелпера, `goals_part[ref]` для «вчера», форма `days` неизменна)
...
- **`.obsidian-docs/knowledge-bank/patterns/` — НОВЫЙ урок, формулировка РАСШИРЕНА по заметке критика v3.** Прежняя формулировка v3 («хелперы, принимающие `window_dates`, определены только для непрерывного окна и точечно не переиспользуются») **недостаточна**: она закрывает два первых случая (`_forecast_balances`, критика v1; `_payments_tail_by_day`, критика v2) и **пропускает третий** (`_goals_part_by_day`, критика v3 №2), потому что тот от состава списка честно не зависит и всё равно даёт для нового ключа число по чужому месяцу. Урок пишется как **два обязательных вопроса** к оконному хелперу:
  1. **Зависит ли результат от состава переданного списка?** (`_payments_tail_by_day` — да: суффиксная сумма накапливается проходом по списку; `_goals_part_by_day` — нет).
  2. **Для каких дат выполняются посылки функции?** (`_goals_part_by_day` берёт границы месяца по самому дню `D`, поэтому дата из прошлого месяца даёт формально посчитанное, но бессмысленное число — и её докстринг предупреждает об этом прямым текстом).
  Приём проверки, ради которого урок и пишется: **прежде чем добавить дату в список оконного хелпера, прочитать его Note-секции на предмет допущений о диапазоне** — тело может быть корректно, а неверна применимость. Плюс прежний приём: **прежде чем утверждать поведение приватного хелпера, открыть его тело**, а не подпись и не докстринг. Кусок 3 и карточки других страниц зайдут в `money_layers_service.py` снова
- **`.obsidian-docs/knowledge-bank/patterns/` — вторая запись (critique-v3 №1):** «**прежде чем передать сессию в конструктор сервиса, проверить, что `__init__` у него есть**». `AllocationService` — единственный сервис проекта без `__init__`; `AllocationService(session)` даёт `TypeError`, который внутри блочного `except` превращается в молчаливую деградацию карточки. Проверка стоит один `grep "def __init__"` по файлу сервиса
...
8b. **Резерв «вчера» (v4, critique-v3 №2)**: **тест на значение `yesterday["reserve_configured"]`** на фикстуре «`ref` = 1-е число + savings в прошлом месяце + savings в текущем» зелёный; `yesterday["reserve_configured"] == days[0]["reserve_configured"]` — и на фикстуре 1-го числа, и в середине месяца; **mutation-проверка**: замена `goals_part.get(ref, …)` на `goals_part.get(yday, …)` (вместе с передачей `calc_dates` в `_goals_part_by_day`) КРАСНИТ тест. Вручную 1-го числа на наполненной базе: «Свободно» у «Вчера» и у «Сегодня» отличаются только на платежи и баланс дня, не на резерв.
...
11. **AC-5 (R17)**: тесты пустой базы **в двух вариантах** — с `User(id=1)` и без него; в обоих все пять блоков `EMPTY`, `goals` не `FAILED`, в логах нет трейсбека по `goals`. **Плюс контракт `_empty_*` на уровне данных** (v4, №5): `others_summary == ""`, `cushion_label == ""`, `top_goal_progress == 0.0`, `cushion_progress == 0.0`, `month_total == Decimal("0")`, `structure == []`, `total_count == 0`; Optional-поля `None`; `href`-поля непусты.
11b. **`goals=OK` с целями (v4, R20, critique-v3 №1)**: `pytest tests/test_panel_service.py -k goals_ok` — фикстура «два активных цели + `monthly_savings_budget` > 0» даёт `status == OK`, `others_behind_count == 1`, `top_goal_name` = цель с `priority=1`; в логах нет трейсбека по `goals`. Вручную на базе с целями: карточка Цели показывает цифры, а не деградацию.
...
14. **NFR-1**: замер `_load_dashboard_components` (время И число SQL-запросов) + **отдельный замер `render_sidebar_slot("/calendar", None)`**, запись в протокол, сверка с 13 мс куска 1; счётчик `get_money_layers == 1`. **В протокол — три числа с явными границами** (v4, №3): (а) композитор — время и запросы; (б) онбординг-тост `toggle_balance_toast` (`dashboard.py:1191`) — **+1 сессия, +1 SELECT** на ту же загрузку `/dashboard`, вне scope по FR-4, существует и до куска 2; (в) сайдбар на переходе. Формулировка «одна сессия на открытие `/dashboard`» в протокол **не пишется** — она неверна.
...
18. **RTM #62 на живой базе (v4, critique-v3 №6)**: открыть дашборд на базе, где **вчера операции были**. Прочитать три окошка глазами пользователя: не читается ли постоянно пустая подпись у «Вчера» как ложное «вчера операций не было»? **Решение принимается здесь и записывается в протокол:** либо подпись у «вчера» остаётся пустым местом (как в эскизе, `&nbsp;`), либо `.pnl-day-note` у «вчера» **не рендерится вовсе**, а вертикальный ритм трёх окошек держится `min-height` контейнера. Второй вариант предпочтителен, если пустое место читается как утверждение. Молча оставить `&nbsp;`-распорку нельзя — это и есть тот класс «пользователь домыслил цифру», от которого лечит FR-6.


## Учтённые замечания из критики v3

| # | Замечание критики v3 | Решение в v4 |
|---|---|---|
| 🟡 1 | `AllocationService(self.session)` — у класса нет `__init__`, вызов упадёт `TypeError` внутри блочного `except` → карточка Цели в `FAILED` молча на любой базе с целями; в наборе шага 4 нет теста, обязанного от этого покраснеть | **Перепроверено по телу, критик прав.** `app/services/allocation_service.py`: `def __init__` в файле **отсутствует вообще** — класс объявлен на `:18`, первый метод `calculate_allocation(self, goals, monthly_budget, savings_mode="free")` на `:25-30`; сессию сервис не хранит и работает исключительно на переданном списке ORM `Goal` (тело читает `goal.priority`, `.status`, `.monthly_contribution`, `.id`, `.name`, возвращает `AllocationSummary`). Все **пять** вызовов в `app/` — `AllocationService()` (`goals.py:1773`, `:2520`, `:3254`, `:3317`, `redistribution_service.py:38`); в тестах — ещё восемь, тем же способом. Исправлено на **`AllocationService().calculate_allocation(goals, monthly_budget, savings_mode)`** с комментарием у вызова, почему без аргументов. Строка **«Зависимости» разделена на две категории**: session-сервисы (`MoneyLayersService`, `GoalService`, `DashboardService`, `AnalyticsService`, `WishlistService` — `Service(self.session)`) и **stateless** (`AllocationService` — без аргументов, `__init__` нет). Там же оговорка **«stateless ≠ вне сессии»**: метод читает живые ORM-объекты, поэтому вызов обязан быть внутри открытой сессии — в `_goals_block` это и так так. В стратегии загрузки строка `AllocationService` переписана с тем же уточнением (0 запросов, но конструктор без сессии). **В шаг 4 добавлен целевой тест `goals=OK`**: `User` с `monthly_savings_budget = 30 000`, две активные цели `priority=1/2` с взносами так, что бюджета хватает первой и не хватает второй → `status == OK`, `others_count == 1`, **`others_behind_count == 1`**, `top_goal_name` — цель с `priority=1`, `top_goal_current`/`top_goal_target` — `Decimal`; плюс ассерт «в логах нет трейсбека по `goals`». Почему такого теста не было в v3: обе фикстуры AC-5 пустые (до `AllocationService` дело не доходит), тест деградации ожидает `FAILED` как **правильный** результат, тесты карточек кормятся словарями. Новый тест краснеет и на конструкторе, и на опечатке в разборе `allocation["results"]`. Риск **R20**; правило в `patterns/`: «прежде чем передать сессию в конструктор, проверить, что `__init__` у него есть» |
| 🟡 2 | `goals_part[yday]` 1-го числа считается по чужому месяцу: `month_start`/`month_end` берутся по самому дню `D`, `savings_by_date` содержит из прошлого месяца один день, `committed` пуст → полный бюджет текущей настройки, отнесённый к прошлому месяцу; «вчера» и «сегодня» разойдутся по резерву — FR-6 в миниатюре | **Принят предпочтительный вариант критика: `_goals_part_by_day` остаётся на `window_dates`, `_yesterday_slice` берёт `goals_part[ref]`.** Перепроверено по телу целиком, включая Note-секции: цикл `for day in window_dates` (`:579`), `month_start = _month_start(day)` / `month_end = _month_end(day)` (`:580-581`), фильтры `month_start <= op_date <= day` / `day < op_date <= month_end` (`:588-591`), `result[day] = max(0, monthly_budget − consumed − committed)` (`:594-595`); **Note `:507-511`** прямым текстом называет `reference_date == 1-е число` вырожденным случаем. То есть нарушалась **посылка о диапазоне**, а не тело — и именно поэтому проверка v3 («зависит ли результат от состава списка») прошла мимо. **Обоснование выбора:** (а) ключа с нарушенной посылкой не возникает **вовсе**, а не «обрабатывается ветвлением» — вырожденный случай исчезает без `if` по датам; (б) докстринг хелпера остаётся верным буквально, без оговорок; (в) вызов куска 1 **не меняется совсем** — C-3 соблюдается строже, чем в v3; (г) единственность **источника** сохраняется: резерв «вчера» и резерв «сегодня» — это **одно и то же число из одного словаря**, поэтому расхождение между ними становится **физически невозможным**, а не просто согласованным — для FR-6 это строже, чем v3. **Проверено, что вариант корректен во ВСЕ дни месяца, а не только 1-го:** `goals_part[ref]` всегда считается по границам месяца `ref` (`_month_start(ref)`…`_month_end(ref)`), а `savings_by_date` покрывает весь этот месяц по построению `collect_start = min(_month_start(ref), yday)` — то есть посылка функции для ключа `ref` выполнена в любой день, включая 1-е и последний. Альтернатива «`yesterday = None` 1-го числа» отклонена: она даёт прочерк там, где баланс и платежи «вчера» посчитаны корректно, то есть теряет данные из-за одного слагаемого, и вводит ветвление по датам. **Цена названа прямо** в докстринге `_yesterday_slice`: `reserve_configured` «вчера» — это **настройка периода**, а не реконструкция прошлого месяца; реконструировать нечем — истории бюджета в схеме нет (C-4, `users.monthly_savings_budget` — одна настройка на все месяцы, `database.py:99`), что уже зафиксировано Note «ДОПУЩЕНИЕ „БЮДЖЕТ НЕ МЕНЯЛСЯ ВНУТРИ МЕСЯЦА"». **Строка в таблице границ** — две: `_goals_part_by_day` (вызов) «**без изменений**, с причиной» и новая `_yesterday_slice.reserve_configured` = `cushion_threshold + goals_part[ref]`. **Тест на значение** (шаг 1, п. 8b, симметричен тесту `payments`): фикстура «`ref` = 1-е число + savings в прошлом месяце, включая `yday` + savings в текущем» → (а) `yesterday["reserve_configured"] == days[0]["reserve_configured"]`; (б) числовое равенство `cushion_threshold + max(0, budget − consumed(ref) − committed(ref))` по **текущему** месяцу; (в) mutation-проверка (`goals_part.get(yday)` + `calc_dates` красят тест); (г) тот же ассерт (а) в середине месяца. RTM #93, RTM #85/#89 уточнены, R19 переформулирован |
| 🟡 3 | Четвёртая сессия рендера не учтена: `toggle_balance_toast` (`dashboard.py:1191`) висит на `url.pathname` + `profile-updated` с `prevent_initial_call=False`; формулировка «четвёртой не было» неверна | **Перепроверено, критик прав.** `grep get_db_session app/components/dashboard.py` даёт **три** попадания: `:871`, `:1002`, `:1191`. Третье — внутри `toggle_balance_toast` (`:1169-1200`): Input'ы `url.pathname`, `balance-alert-toast.is_open`, `profile-updated.data`, `State("balance-toast-dismissed")`, `prevent_initial_call=False`; при `pathname == "/dashboard"` и не-dismissed открывает сессию под `OnboardingService(session).get_status(DEFAULT_USER_ID)`. Фраза «четвёртой не было» **убрана**. Стало: **«сессий НА ПУТИ СБОРКИ ЩИТКА: 1 (было 3)»** + **отдельный абзац** «Сессия онбординг-тоста — ВНЕ SCOPE, но в бюджете NFR-1»: сохраняется намеренно (FR-4 требует тост не трогать), но в стоимость открытия страницы входит и названа явно, чтобы цифра «1» не поехала в knowledge-bank как «одна сессия на открытие `/dashboard`». Добавлена **итоговая картина по путям, а не «за рендер»**: открытие `/dashboard` — 1 (щиток) + 1 (тост, вне scope) + 0 (сайдбар, `[]` до открытия сессии); переход на раздел — 1 (сайдбар) + сессии раздела; правка профиля — 1 (сайдбар) + 1 (тост). Было 3 + 1 = 4, стало 1 + 1 = 2. **Замер шага 12 уточнён**: мерится **только** путь сборки щитка (`_load_dashboard_components`), и это сказано прямо; в протокол пишутся **три** числа — композитор, «+1 сессия и +1 SELECT: тост, вне scope», сайдбар на переходе; формулировка «одна сессия на открытие `/dashboard`» в протокол не пишется. RTM #95; в `modules/services.md` — с той же оговоркой |
| 🟢 4 | Ветка ошибки `load_and_navigate_calendar` (`:938-948`) возвращает `state` без изменений — `focus_applied_ts` не запишется; стоит сказать, что это осознанно | **Зафиксировано одной строкой в шаге 9 и строкой RTM #97.** Сверено по телу: `except Exception` (`:937-948`) возвращает исходный `state` четвёртым элементом (`:944`). Ветка **не правится**, `focus_applied_ts` в неё **намеренно не дописывается**. Поведение безопасное: `ts` не сохранён, но переприменения не будет, потому что при следующем срабатывании по любому **другому** Input'у **первое** условие guard'а (`ctx.triggered_id == Store`) уже не выполнится. Дописывать `ts` в ветку ошибки не просто не нужно, а **вредно**: (а) второе место записи одного факта; (б) после сбоя загрузки повторный клик по той же двери как раз **должен** сработать — а запись `ts` в ветку ошибки его бы съела. Фиксация нужна именно для того, чтобы при реализации не возникло соблазна «дописать ts и в ветку ошибки» |
| 🟢 5 | `_empty_goals()` используется, но её контракт не задан: какие значения у не-Optional `top_goal_progress: float`, `others_summary: str`, `cushion_progress: float`; тот же вопрос к остальным `_empty_*` | **Задан контракт для всех пяти блоков** (секция после сигнатур сервиса). Правило одно: **Optional → `None`; числовые не-Optional → `0`/`Decimal("0")`/`0.0`; строковые не-Optional → `""`; списки → `[]`; `bool` → `False`; `href` и `month_label` — заполняются как обычно** (маршрут двери и подпись периода — не цифры; дверь обязана работать и на пустой карточке, FR-2). Полный докстринг `_empty_goals` с перечислением **каждого** поля: `top_goal_progress=0.0`, `top_goal_current=Decimal("0")`, `top_goal_target=Decimal("0")`, `others_count=0`, `others_behind_count=0`, **`others_summary=""`**, `cushion_is_configured=False`, `cushion_progress=0.0`, **`cushion_label=""`**; Optional (`top_goal_id`, `top_goal_name`, `top_goal_target_date`, `top_goal_href`) → `None`. Плюс таблица по остальным четырём: `_empty_calendar` (`days=[]` — **не** три окошка с нулями, `dip_is_strong=False`, `dip_*` → `None`), `_empty_operations` (`recent=[]`, `upcoming=[]`, оба `href` как обычно), `_empty_analytics` (`month_label` как обычно, `month_total=Decimal("0")`, `top_category_total=Decimal("0")`, `top_category_share=0.0`, `structure=[]`, `top_category_name=None`), `_empty_wishlist` (`items=[]`, `total_count=0`). **Почему `""`, а не текст:** при `EMPTY` build-функция рисует **свой** текст пустого состояния и в `others_summary`/`cushion_label` не смотрит (`status` — единственный источник правды отрисовки, RTM #81); текст в данных завёл бы второй источник формулировки, а строка вида «0 ₽»/«0%» при будущей правке build-функции протекла бы в дерево как числовой артефакт, запрещённый AC-5. Также назван **второй путь в `EMPTY`** у `_goals_block` (нет активных целей И `cushion_target` пуст) — v3 его в докстринге не упоминала. **Тест на уровне данных** в шаг 4: тест AC-5 «нет `₽`/`%`» смотрит дерево и подмену `""` на текст не поймает. RTM #96 |
| 🟢 6 | RTM #62: подпись окошка «вчера» останется пустой **всегда** (`payments` фильтруется `day >= reference_date`), и пользователь прочтёт пустоту как «вчера операций не было»; проверить на живой базе и решить, не убрать ли место под подпись совсем | **Строка RTM #62 переписана: «пустая ВСЕГДА», а не «когда операций не было».** Механизм назван по телу (`:404`, фильтр `day >= reference_date` → вчерашних операций в модели нет **по построению**, `operations_note = None` для «вчера» на любой базе), и прямо сказано следствие: пользователь может прочитать постоянную пустоту как **ложное утверждение**. Эскиз ставил `&nbsp;` — формально по эскизу, но решение принимается **по живой базе, а не по эскизу**. **Пункт ручного чек-листа 18**: открыть дашборд на базе, где **вчера операции были**, прочитать три окошка глазами пользователя, и записать решение в протокол — либо пустое место остаётся (как в эскизе), либо **`.pnl-day-note` у «вчера» не рендерится вовсе**, а вертикальный ритм трёх окошек держится `min-height` контейнера вместо `&nbsp;`-распорки. Второй вариант помечен предпочтительным, если пустота читается как утверждение. Отдельно сказано: **молча оставить распорку нельзя** — это тот же класс «пользователь домыслил цифру», от которого лечит FR-6, и развилка фиксируется в RTM, чтобы её не закрыли молча в CSS |

## Ответы на вопросы критика v3

**1. `[факт]` Чему равен `goals_part[yday]`, когда `reference_date` — 1-е число месяца, и какое число попадёт в `yesterday["reserve_configured"]`?**

Разбор критика верен буквально; перепроверено по телу `_goals_part_by_day` (`money_layers_service.py:578-597`) и по её Note-секциям (`:492-577`).

Тело для каждого дня `D` списка:

```python
for day in window_dates:
    month_start = _month_start(day)      # :580 — по САМОМУ дню D
    month_end   = _month_end(day)        # :581 — по САМОМУ дню D
    consumed = committed = Decimal("0")
    for op_date, amount in savings_by_date.items():
        if   month_start <= op_date <= day:  consumed  += amount    # :588-589
        elif day < op_date <= month_end:     committed += amount    # :590-591
    goals = monthly_budget - consumed - committed                   # :594
    result[day] = goals if goals > 0 else Decimal("0")              # :595
```

Если бы `yday` попал в список при `ref` = 1-е число (то есть `yday` = последний день прошлого месяца), то для ключа `yday`:

- `month_start(yday)`, `month_end(yday)` — границы **прошлого** месяца;
- `consumed(yday)` = Σ по `[month_start(yday), yday]`, но `savings_by_date` собран от `collect_start = min(_month_start(ref), yday) = yday`, то есть из прошлого месяца содержит **ровно один день** — сам `yday`; остальные ~30 дней в сбор не входят;
- `committed(yday)` = Σ по `(yday, month_end(yday)]` = **пусто**, потому что `yday == _month_end(yday)`;
- итог: **`goals_part[yday] = max(0, monthly_budget − savings(yday))`** — полный бюджет **текущей настройки** минус операции одного дня, отнесённый к чужому месяцу, в котором взносы могли быть уже полностью сделаны.

Следствие в UI: 1-го числа «Вчера» показало бы резерв ≈ полный месячный бюджет и заниженное «Свободно», причём посчитанные **по другому месяцу**, чем соседнее «Сегодня». Направление ошибки безопасное (занижаем свободное), но цифра неверна, а нарушение — ровно «два соседних числа по разной логике», от которого лечит FR-6. Это **не дефект функции**: её докстринг (`Note :507-511`) прямым текстом называет `reference_date == 1-е число` вырожденным случаем — то есть нарушалась бы её **посылка о диапазоне**, а не тело.

**Что попадёт в `yesterday["reserve_configured"]` в v4: `cushion_threshold + goals_part[ref]`** — ключом **сегодняшнего** дня, потому что ключа `yday` в словаре нет вовсе: `_goals_part_by_day` вызывается на `window_dates` (вызов куска 1 не меняется). Формула развёрнуто:

```
reserve_configured(yesterday)
  = cushion_threshold
  + max(0, monthly_budget
           − Σ savings_by_date[d] для d ∈ [_month_start(ref), ref]
           − Σ savings_by_date[d] для d ∈ (ref, _month_end(ref)])
  ≡ reserve_configured(days[0])            # тождественно, одно число из одного словаря
```

**Корректность во все дни месяца, не только 1-го** (проверено по телу, а не по примеру): `goals_part[ref]` всегда считается по границам месяца самого `ref`, а `savings_by_date` покрывает весь этот месяц по построению `collect_start = min(_month_start(ref), yday)` — для любого `ref` `collect_start <= _month_start(ref)`. Значит посылка функции для ключа `ref` выполнена **в любой день месяца**, включая 1-е и последний; ветвления по датам нет, вырожденного случая нет.

**Почему выбран этот вариант, а не `yesterday = None` 1-го числа.** Прочерк выбросил бы окошко целиком в день, когда баланс и платежи «вчера» посчитаны **корректно**, — то есть потерял бы данные из-за одного слагаемого, и завёл бы `if` по датам в `_yesterday_slice`. Вариант критика вместо этого делает расхождение резерва «вчера»/«сегодня» **физически невозможным** (одно число, один словарь, один ключ), что для FR-6 строже, чем «согласовано». Возражение «теряется один список на оба хелпера» снимается тем, что аргумент v3 был про единственность **источника**, а не объекта: источник как раз становится строго единственным.

**Цена названа прямо**, в докстринге `_yesterday_slice`: `reserve_configured` «вчера» — это **настройка периода**, а не реконструкция прошлого месяца. Реконструировать было бы нечем: истории бюджета в схеме нет (C-4, `users.monthly_savings_budget` — одна настройка на все месяцы, `database.py:99`), что уже зафиксировано Note «ДОПУЩЕНИЕ „БЮДЖЕТ НЕ МЕНЯЛСЯ ВНУТРИ МЕСЯЦА"» в самой функции.

**Тест на значение** (шаг 1, п. 8b; симметричен тесту на `payments`): фикстура «`ref` = 1-е число + `monthly_savings_budget` > 0 + savings-операции в прошлом месяце, включая его последний день = `yday` + savings-операция в текущем месяце». Ассерты: (а) `yesterday["reserve_configured"] == days[0]["reserve_configured"]`; (б) числовое равенство приведённой формуле, считанной **по текущему** месяцу — то есть операции прошлого месяца в резерв «вчера» **не входят**; (в) mutation-проверка: замена на `goals_part.get(yday, …)` вместе с передачей `calc_dates` в хелпер обязана покрасить тест (и по отдельности тоже — обе части проверяются); (г) тот же ассерт (а) на фикстуре с `ref` **в середине месяца**, чтобы утверждение «корректно во все дни» было проверено, а не заявлено.

---

**2. `[факт]` Каким вызовом создаётся `AllocationService` в `_goals_block`?**

**`AllocationService()` — без аргументов.** Критик прав, v3 ошибалась.

Перепроверено по телу `app/services/allocation_service.py`: `def __init__` в файле **отсутствует вообще**. Структура файла: модульная константа `SAVINGS_MODE_MULTIPLIERS` (`:11-15`), `class AllocationService` (`:18`), докстринг класса, и **сразу** `def calculate_allocation(self, goals: list[Goal], monthly_budget: Decimal, savings_mode: str = "free") -> AllocationSummary` (`:25-30`). Сервис не хранит ни сессии, ни какого-либо состояния: тело метода читает только переданный список ORM `Goal` (`goal.priority` для сортировки, `goal.status`, `goal.monthly_contribution`, `goal.id`, `goal.name`) и возвращает `AllocationSummary` — ни одного обращения к `self.<что-либо>`.

Все **пять** вызовов в `app/` конструируют его без аргументов: `goals.py:1773`, `:2520`, `:3254`, `:3317`, `redistribution_service.py:38` (`allocation_service or AllocationService()`); в тестах — ещё восемь мест, тем же способом. `AllocationService(self.session)` дал бы `TypeError: AllocationService() takes no arguments` **внутри блочного `except`** `_goals_block` → `CardStatus.FAILED` на **любой** базе с целями, молча, с трейсбеком в лог и живым дашбордом. Ровно тот профиль дефекта, от которого v3 защищалась в двух других местах.

**Важная оговорка, которая идёт вместе с исправлением: stateless ≠ «можно вызывать вне сессии».** `goals` — живые ORM-объекты, и `calculate_allocation` читает их атрибуты, поэтому вызов обязан находиться внутри открытой сессии, иначе `DetachedInstanceError`. Для `_goals_block` это выполнено по построению (весь блок работает внутри `with get_db_session()` композитора), но записать нужно: иначе «сессия не нужна» однажды прочитается как «можно после `with`».

**Где исправлено:** код `_goals_block`, шаг 3 плана, строка «Зависимости» (разделена на session-сервисы и stateless), стратегия загрузки, RTM #94, риск R20, и правило в `patterns/`: «прежде чем передать сессию в конструктор сервиса, проверить, что `__init__` у него есть» — `AllocationService` в проекте единственный такой, и проверка стоит один `grep "def __init__"`.

**Тест, который обязан покраснеть от этой ошибки** (в v3 такого не было ни одного): шаг 4, `goals=OK` на фикстуре с `monthly_savings_budget = 30 000`, `savings_mode="free"` и **двумя** активными целями `priority=1/2` с взносами так, что бюджета хватает первой и не хватает второй → `status == OK`, `others_count == 1`, `others_behind_count == 1`, `top_goal_name` — цель с `priority=1`, `top_goal_current`/`top_goal_target` типа `Decimal`; плюс ассерт «в логах нет трейсбека по `goals`». Почему прежний набор проходил мимо: обе фикстуры AC-5 пустые (`goals=EMPTY`, до `AllocationService` дело не доходит), тест деградации ожидает `FAILED` как **правильный** результат, тесты карточек кормятся словарями. Этот же тест ловит и опечатки в разборе `allocation["results"]`.

---

**3. `[факт]` Входит ли сессия `toggle_balance_toast` (`dashboard.py:1191`) в замер NFR-1 шага 12, и остаётся ли она после куска 2?**

**Остаётся — намеренно. В замер шага 12 не входит — и это теперь сказано явно, отдельной строкой протокола.** Формулировка v3 «четвёртой не было» неверна; критик прав.

Перепроверено: `grep get_db_session app/components/dashboard.py` → **три** попадания, `:871`, `:1002`, `:1191`. Третье — внутри `toggle_balance_toast` (`:1169-1200`): Output `balance-alert-toast.is_open`, Input'ы `url.pathname`, `balance-alert-toast.is_open`, `profile-updated.data`, State `balance-toast-dismissed.data`, **`prevent_initial_call=False`**. При `pathname in ("/dashboard", "/")` и не-dismissed открывает сессию под `OnboardingService(session).get_status(DEFAULT_USER_ID)` и возвращает `status["needs_balance_alert"]`; при сбое — `except Exception: return False`. То есть сессия открывается на **каждой** загрузке дашборда и ещё раз на каждой правке профиля (Input `profile-updated`).

**Остаётся ли:** да. FR-4 требует онбординг-тост не трогать, его логика к щитку отношения не имеет, и кусок 2 её не касается. Но в стоимость открытия страницы она входит.

**Как переписано.** Вместо «сессий за рендер дашборда: 1 (было 3)… Четвёртой не было» — **«сессий НА ПУТИ СБОРКИ ЩИТКА: 1 (было 3)»** плюс отдельный абзац «Сессия онбординг-тоста — ВНЕ SCOPE, но в бюджете NFR-1» с номерами строк и указанием, что она сохраняется намеренно. Плюс **итоговая картина по путям**, а не «за рендер»:

- открытие `/dashboard`: **1** (сборка щитка, `:1002`) + **1** (тост, `:1191`, вне scope) + **1** (`onboarding_wizard.py:153`, вне scope) + **0** (сайдбар: `render_sidebar_slot` возвращает `[]` **до** открытия сессии) = **3**;
- переход на `/calendar|/goals|/transactions|/analytics`: **1** (сайдбар) + **1** (визард `onboarding_wizard.py:153`) + сессии самого раздела (куском 2 не меняются);
- правка профиля: **1** (сайдбар) + **1** (тост), оба по `profile-updated`.

**Уточнение v4-fix (critique-v4, №1): было НЕ 4, а 6.** Перечень v4 строился тем же способом, что в v2 и v3 — `grep` по одному файлу, — и снова оказался неполон. Прогон `grep -rn get_db_session app/components/ app/main.py` добавляет две сессии на тех же путях: `onboarding_wizard.py:153` (`check_onboarding_and_validate`, Input `url.pathname`, `prevent_initial_call=False`, **guard'а по pathname нет** — при `triggered_id == "url"` сессия открывается на любом переходе) и `sidebar.py:178` (`update_sidebar_profile` — сайдбар стоит **статически** в layout `main.py:58-61`, поэтому колбэк срабатывает и на дашборде). Итого до куска 2 на открытии `/dashboard`: `dashboard.py:1002` + `:871` + `wishlist.py:33` + `:1191` + `onboarding_wizard.py:153` + `sidebar.py:178` = **6**. Стало **3** (из них 2 — вне scope по FR-4).

**Побочный вывод, который был потерян:** выигрыш Подхода B **больше** заявленного. `sidebar.py:178` с дашборда уходит совсем (сайдбара там нет), а на разделах одна сессия `render_sidebar_slot` заменяет и чтение профиля, и то, что понадобилось бы для подсветки. То есть Подход B не «добавляет сессию на переходах», а перераспределяет уже существующую.

**Метод вместо числа.** В план шага 12 внесена не цифра, а **команда**: `grep -rn get_db_session app/components/ app/main.py` с разбором каждого попадания по признаку «срабатывает ли колбэк на данном пути». Три итерации перечень расширялся ровно настолько, насколько указывал критик, потому что метод оставался прежним — это тот же рецидив, что с оконными хелперами, в другом материале.

**Что именно мерит шаг 12:** только **путь сборки щитка** — `time.perf_counter` вокруг `_load_dashboard_components` + счётчик SQL через `sqlalchemy.event` на `before_cursor_execute`. Тост в эту цифру **не попадает** (он отдельный колбэк). Поэтому в протокол пишутся **три** числа с явными границами: (а) композитор — время и запросы; (б) «+1 сессия, +1 `SELECT User`: онбординг-тост `dashboard.py:1191`, вне scope по FR-4, существует и до куска 2»; (в) `render_sidebar_slot("/calendar", None)` — цена Подхода B на переходе. Формулировка «одна сессия на открытие `/dashboard`» в протокол и в `modules/services.md` **не пишется** — она неверна, и именно в таком виде цифра и поехала бы в knowledge-bank как факт (причина, по которой критик поднимал это и в v2). RTM #95.

---

**4. `[факт]` Какие значения кладёт `_empty_goals()` в поля `top_goal_progress: float`, `others_summary: str`, `cushion_progress: float`, объявленные не-Optional?**

**`top_goal_progress = 0.0`, `others_summary = ""`, `cushion_progress = 0.0`.** Полный контракт `_empty_goals()`:

| Поле | Значение при `EMPTY` |
|---|---|
| `status` | `CardStatus.EMPTY` |
| `top_goal_id`, `top_goal_name`, `top_goal_target_date`, `top_goal_href` | `None` (Optional по контракту) |
| `top_goal_progress: float` | **`0.0`** |
| `top_goal_current`, `top_goal_target` | `Decimal("0")` |
| `others_count`, `others_behind_count` | `0` |
| `others_summary: str` | **`""`** — пустая строка, **не** «Нет целей» |
| `cushion_is_configured: bool` | `False` |
| `cushion_progress: float` | **`0.0`** |
| `cushion_label: str` | **`""`** — пустая строка, **не** «0%» |

**Почему `""`, а не осмысленный текст.** При `status == EMPTY` `build_goals_card` рисует **свой** текст пустого состояния и в `others_summary`/`cushion_label` не смотрит вовсе — `status` объявлен единственным источником правды отрисовки (RTM #81, решение владельца «каждая карточка честна сама за себя»). Текст в данных завёл бы **второй** источник формулировки пустого состояния. А строка вида `"0 ₽"` или `"0%"` в данных при будущей правке build-функции протекла бы в дерево как числовой артефакт, прямо запрещённый AC-5 — то есть выбор `""` тут не косметика, а защита инварианта.

**Остальные четыре `_empty_*` — по одному общему правилу:** Optional → `None`; числовые не-Optional → `0` / `Decimal("0")` / `0.0`; строковые не-Optional → `""`; списки → `[]`; `bool` → `False`; **`href`-поля и `month_label` заполняются как обычно** (маршрут двери и подпись периода — не цифры; дверь обязана работать и на пустой карточке, FR-2 «находимость разделов» от наличия данных не зависит).

- `_empty_calendar()`: `days=[]` — **не** три окошка с нулями (при `EMPTY` карточка рисует своё пустое состояние, а нули в данных однажды протекли бы); `dip_is_strong=False`; `dip_date`/`dip_free`/`dip_href` → `None`. Плюс уже принятая оговорка #91: `dip_*` не рисуются при `status != OK`, **даже если непустые**.
- `_empty_operations()`: `recent=[]`, `upcoming=[]`, `recent_href`/`upcoming_href` — как обычно. Optional-полей нет.
- `_empty_analytics()`: `month_label` — как обычно («августа»: это подпись периода, а не цифра, и пустому состоянию она нужна тоже); `month_total=Decimal("0")`, `top_category_total=Decimal("0")`, `top_category_share=0.0`, `structure=[]`, `href` как обычно; `top_category_name=None`.
- `_empty_wishlist()`: `items=[]`, `total_count=0`. Optional-полей нет.

**Что именно проверит тест.** Критик прав, что от этого зависит форма теста: тест AC-5 «нет `₽`/`%`» смотрит **дерево**, а не данные, и подмену `""` на текст не поймает. Поэтому в шаг 4 добавлен отдельный тест **на уровне данных**: на фикстуре пустой базы для каждого из пяти срезов проверяется не только `status == EMPTY`, но и что все не-Optional поля равны перечисленным нейтральным значениям (`others_summary == ""`, `cushion_label == ""`, `top_goal_progress == 0.0`, `cushion_progress == 0.0`, `month_total == Decimal("0")`, `structure == []`, `total_count == 0`), Optional-поля — `None`, а `href`-поля непусты. Плюс попутно зафиксирован **второй путь в `EMPTY`** у `_goals_block`, которого v3 в докстринге не упоминала: пользователь есть, но нет ни одной активной цели **и** `cushion_target` не задан.
