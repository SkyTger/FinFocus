...
2. **Коммит правок** (если были):
   ```bash
   git add . && git commit -m "chore: final QA fixes [protocol-0021/06]"
   ```
...
5. **Финальный коммит протокола:**
   ```bash
   git add .protocols/0021-dashboard-foundation/ && git commit -m "docs(protocol): finalize 0021-dashboard-foundation [protocol-0021/06]"
   git push
   ```
...
```
(Протокол 0021-dashboard-foundation — Финализация):
