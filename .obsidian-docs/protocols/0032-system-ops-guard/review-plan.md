# Review and Merge Plan: 0032-system-ops-guard

## Paths

- **Project root**: `/home/skytiger/Projects/FinFocus`
- **Worktree**: `/home/skytiger/Projects/worktrees/0032-system-ops-guard`
- **Protocol artifacts**: `.obsidian-docs/protocols/0032-system-ops-guard`
- **PR**: #32
- **Main branch**: `main`
...
2. Коммит в feature-ветку: `chore(review): step X-m for 0032-system-ops-guard`
...
- Проблемы → исправления в ветке фичи с тегом `[protocol-0032-system-ops-guard/2-m-fix]`

### 3-m. Code Review
1. Сверь `plan.md` и `log.md` с фактом
2. `git diff origin/main...0032-system-ops-guard`
...
2. Task(spec-fidelity-validator): в промпт ТОЛЬКО путь к spec.md,
   project root/worktree, `main` и `0032-system-ops-guard`.
   НЕ передавай plan.md/log.md/выводы шага 3-m — верификатор смотрит
   на код чистыми глазами.
3. Полный отчёт → `.obsidian-docs/protocols/0032-system-ops-guard/fidelity-report.md`; человеку —
   вердикты НЕ ПОКРЫТО / СДЕЛАНО ИНАЧЕ. Advisory: мерж не блокирует,
   судит человек.
4. Признано дефектом → fix в ветке фичи с тегом
   `[protocol-0032-system-ops-guard/3.5-m-fix]`; принято как есть →
   фиксация в `review-log.md`.

### 4-m. Knowledge Bank
```bash
/kb-update .obsidian-docs/protocols/0032-system-ops-guard/plan.md .obsidian-docs/protocols/0032-system-ops-guard/log.md
```
...
Затем:
```bash
git add .obsidian-docs/knowledge-bank/ && git commit -m "docs(knowledge-bank): update after 0032-system-ops-guard [protocol-0032-system-ops-guard/4-m]"
git push origin 0032-system-ops-guard
```
...
```bash
git add -A && git commit -m "docs: update after 0032-system-ops-guard [protocol-0032-system-ops-guard/4.5-m]"
git push origin 0032-system-ops-guard
```
...
git merge --no-ff 0032-system-ops-guard
...
1. Убедись что все коммиты запушены: `git push origin 0032-system-ops-guard`
...
> `Для завершения выполни /protocol-cleanup 0032-system-ops-guard`
