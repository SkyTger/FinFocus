...
## Шаг 3: Тесты композитора (2026-08-26)

Создан `tests/test_panel_service.py` — 19 тестов в 8 классах, все
sub-tasks шага закрыты: AC-3 (цифры окошек == модель слоёв, оба дня +
href'ы + dip_*); целевой тест goals=OK (двe цели priority 1/2, бюджет
30 000 хватает первой → others_behind_count == 1, «1 отстаёт», плюс
«в логах нет трейсбека по goals»); типы OperationRow (isinstance date,
kind на всех шести значениях enum, is_recurring у материализованного
recurring-инстанса); материализация (чтение всех полей после закрытия
сессии); AC-5 в двух вариантах пустой базы (с User и без — все пять
EMPTY, не FAILED, без трейсбеков); контракт _empty_* на уровне данных
(нейтральные значения, Optional=None, href непусты); деградация
(patch AnalyticsService → analytics FAILED, остальные OK, один
трейсбек в логе; FAILED-срез несёт нейтральные значения); смешанные
случаи (is_empty=True + цель → goals OK / операции есть + целей нет
→ goals EMPTY); согласованность подушки с get_settings (R15);
счётчики (get_money_layers == 1 вызов; SQL-вклад блоков <= 12
запросов через before_cursor_execute).

**Mutation smoke** (3 порчи, все ловятся):
1. `AllocationService(self.session)` → красный целевой тест goals=OK
2. «Завтра» из days[0] вместо days[1] → красный AC-3. **Первый прогон
   мутацию НЕ поймал** — на исходной фикстуре балансы сегодня/завтра
   совпадали; фикстура усилена расходом на завтра + ассерты
   `days[1] != days[0]` и `operations_note == "план"` (RTM #62)
3. raise вместо EMPTY при user is None → красный AC-5 «без пользователя»

Прогон: 19 passed; полный сьют — 712 passed (было 693). black чист,
flake8 без замечаний.

## Шаг 4: Карточки-двери (2026-08-26)

Создан `app/components/panel_cards.py` (~560 строк) — чистые
build-функции пяти карточек + `_door_shell` + `build_cards_row`;
в `app/assets/panel.css` добавлены секции 7-10 (двери, wishlist-полоса,
адаптив 1180/680, prefers-reduced-motion).

**По плану шага**: `_door_shell` — заголовок dcc.Link при href, ни
одного серверного Input (C-6 неуязвим по построению); календарь —
два окошка-ссылки + flagline только при OK с `pnl-flagline-strong`
по факту знака `dip_free <= 0`; цели — топ-цель ссылкой, сводка,
подушка с `margin-top:auto` (RTM #66); операции — группы по
`OPERATIONS_PER_GROUP` со ссылками-заголовками на диапазоны
источника; аналитика — CSS-полоска мини-структуры без Plotly +
подпись объявленного расхождения «расходы {месяца} · без регулярных
и взносов в цели» (RTM #87); wishlist — полоса `id="panel-wishlist-door"`
(уровень 1 — модал через Store, подключение в шагах 6-7), хотелки —
dcc.Link на `/calendar?wishlist_item=<id>` (уровень 2); `build_cards_row`
— пять карточек безусловно, единственный источник правды — status.

**Интерпретации** (задокументированы в докстрингах):
- Тексты EMPTY/FAILED живут в build-функциях (RTM #96): у каждой
  карточки свой смысловой текст без чисел; FAILED — «Не удалось
  загрузить раздел» + подсказка, что дверь-заголовок работает
- Заголовок двери «Операции» ведёт на recent_href (диапазон текущего
  месяца) — в духе FR-3 «фрагмент ведёт в контекст»
- Окошко дня показывает баланс + подпись «свободно N ₽» (поле free
  контракта — «для подписи под остатком»)
- Вёрстка двух окошек: тот же flex-ритм эскиза (ряд забирает высоту
  карточки), комментарий в CSS об отличии от трёхоконного эскиза

**Смоук**: build_cards_row отрендерен на OK/EMPTY/FAILED фикстурах —
без исключений. Попутно в шаге замечено и исправлено отступление
шага 2: `_calendar_block` был обёрнут в try/except вопреки решению
(«чистая функция, без try/except») — убран, докстринг get_panel_data
уточнён, 19 тестов сервиса зелёные.
...
## Шаг 6: Сайдбар без колбэков (2026-08-26)

Подход B реализован полностью:
- `main.py`: `html.Div(id="sidebar-slot")` вместо статического
  `create_sidebar()`; колбэк `render_sidebar_slot(pathname,
  profile_updated)` — два Input'а на всегда присутствующие `url` и
  `profile-updated`, `[]` на дашборде ДО открытия сессии, fail-open
  чтения профиля (заглушка + `logger.opt(exception=True)`, навигация
  не теряется); плюс clientside-триггер аватара сайдбара →
  `open-profile-trigger` (переехал сюда, т.к. sidebar.py стал чистым)
- `sidebar.py`: `create_sidebar(pathname, profile)` — чистая функция;
  литералы «Пользователь»/😊 заменены полями профиля,
  `_build_nav_links(pathname or "/dashboard")`; ОБА колбэка
  (`highlight_active_sidebar`, `update_sidebar_profile`) удалены
  вместе с импортами и `exc_info=True`
- `profile_modal.py`: `Input("sidebar-profile-container")` убран —
  единственный вход открытия `open-profile-trigger` (guard на пустой
  Store сохранён); три `exc_info=True` → `logger.opt(exception=True)`
  (попутный долг 0027)
- `sidebar.css`: один механизм скрытия — `.sidebar-column:empty
  { display: none }` с комментарием-запретом второго механизма
- Тестов на удалённые колбэки не было (grep пуст);
  `test_profile_modal_callbacks.py` адаптирован по плану шага 11
  solution: тест «вход сайдбара сохранён» заменён парой
  «прямого Input нет» + «clientside-триггер аватара зарегистрирован
  в main.py», сигнатура вызовов без `open_clicks`

**Ручная проверка (обязательная, выполнена в живом браузере,
agent-browser, чистая база + наполненная)**:
- профиль открывается шестерёнкой на дашборде И аватаром на каждом
  из четырёх разделов (/calendar /transactions /analytics /goals)
- имя и аватар в сайдбаре корректны на всех разделах; после правки
  профиля (имя «Никита», аватар 🚀) сайдбар обновился без перезагрузки
- подсветка активного пункта корректна на всех четырёх разделах
- сайдбара на дашборде нет, колонка схлопнута (:empty → display:none)
- бонусом проверены шаги 4-5: пустая база — все карточки EMPTY без
  числовых артефактов (скриншот); наполненная — AC-3 глазами (цифра
  окошка «Сегодня» == баланс шапки), подписи «1 операция»/«план»,
  подпись расхождения в Аналитике, дверь Wishlist открывает модал
- замечено поведение источника (не дефект куска 2): операция
  сегодняшнего дня видна и в «Недавних», и в «Предстоящих» — ref
  включён в оба диапазона DashboardService; так было и в прежних
  split-таблицах, C-3 запрещает править сервис
...
## Шаг 8: Тесты UI и адаптация (2026-08-26)

Три новых тестовых файла (+51 тест):
- `tests/test_panel_cards_ui.py` (26) — стиль test_dashboard_panel_ui
  (iter_tree/joined_text/фикстуры-словари/относительные даты, без БД):
  конституция FR-2 (пять карточек при OK/EMPTY/FAILED, двери живы при
  FAILED); AC-5 (пустые состояния без ₽/%/нулей, FAILED без чисел);
  AC-7 в двух фикстурах (минимум >0 — нет strong; <=0 и ==0 — есть);
  целевой тест #91 (EMPTY при непустых dip_* → маркера в дереве НЕТ);
  ДВА окошка «Сегодня»/«Завтра» с ассертом состава (решение владельца);
  href'ы всех дверей + узел panel-wishlist-door; «Доход» отсутствует
  в Аналитике (RTM #13) + наличие подписи объявленного расхождения
  (#87) + мини-структура без Plotly (RTM #70); AC-4 (подушка — строка
  в Целях, ровно 4 гнезда, отдельной карточки нет); смешанные случаи
  пустоты (#81); группы операций, маркер 🔁, знаки сумм по kind
- `tests/test_sidebar.py` (11) — сайдбар впервые покрыт: контракт
  входов render_sidebar_slot (ровно 2 Input на url/profile-updated,
  ни одного на элементы сайдбара); «[] на дашборде БЕЗ открытия
  сессии» (mock-ассерт not_called); регрессионный якорь «в модуле
  sidebar нет ни одного @callback» + отсутствие старых функций;
  чистота create_sidebar (имя/эмодзи из профиля, литералов нет,
  get_db_session в модуле нет); подсветка активного пункта на всех
  четырёх разделах; fail-open профиля (пять пунктов меню при сбое БД)
- `tests/test_panel_query_params.py` (14) — контракт владения
  url.search: /transactions и прочие пути → PreventUpdate; разбор
  focus_date/wishlist_item/open_recon/goal со очисткой search; битые
  значения молча мимо; фиксация «нераспознанные Store'ы → no_update,
  не None» (регрессионная защита дефекта шага 7); различие ts двух
  кликов

Адаптации test_dashboard_callbacks (3 Output) и
test_profile_modal_callbacks (вход через Store) выполнены в шагах 5-6.
47 тестов куска 1 (test_dashboard_panel_ui) зелёные БЕЗ правок.

**Mutation smoke** (2/2 пойманы): (1) сломана оговорка #91 (EMPTY
рисует dip) → красный целевой тест; (2) подпись расхождения урезана
до «расходы августа» → красный тест подписи.

Итог: **764 passed** (713 + 51). black/flake8 чисты.
