# Protocol Context: 0028-money-layers-panel

- **Current Step**: 7
- **Status**: In Progress
- **Last Action**: Шаг 6 — график полос + HTML-легенда с тултипами, дашборд открывается
- **Next Action**: Шаг 7 — переключение callback'ов на новую модель, снятие period-switcher и приветствия, чистка мёртвого кода
- **PR**: https://github.com/SkyTger/FinFocus/pull/28 (Draft)
- **Tech Stack**: `Python 3.10–3.12 + Dash 2.17.1 + dbc + Plotly + SQLAlchemy 2.0.23 + SQLite` (python)
- **Git Branch**: `0028-money-layers-panel`
- **PROJECT_ROOT**: `/home/skytiger/Projects/FinFocus`
- **CWD**: `/home/skytiger/Projects/worktrees/0028-money-layers-panel`
- **Базовый прогон тестов**: 565 (на 2026-08-21)
- **Ожидаемый прогон после шага 7**: 565 − 2 (`TestBuildGreetingText`) + ~37 новых
