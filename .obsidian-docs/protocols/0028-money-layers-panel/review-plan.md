# Review and Merge Plan: 0028-money-layers-panel

## Paths

- **Project root**: `/home/skytiger/Projects/FinFocus`
- **Worktree**: `/home/skytiger/Projects/worktrees/0028-money-layers-panel`
- **Protocol artifacts**: `.obsidian-docs/protocols/0028-money-layers-panel`
- **PR**: #28
- **Main branch**: `main`
...
2. Коммит в feature-ветку: `chore(review): step X-m for 0028-money-layers-panel`
...
- Проблемы → исправления в ветке фичи с тегом `[protocol-0028-money-layers-panel/2-m-fix]`

### 3-m. Code Review
1. Сверь `plan.md` и `log.md` с фактом
2. `git diff origin/main...0028-money-layers-panel`
...
2. Task(spec-fidelity-validator): в промпт ТОЛЬКО путь к spec.md,
   project root/worktree, `main` и `0028-money-layers-panel`.
   НЕ передавай plan.md/log.md/выводы шага 3-m — верификатор смотрит
   на код чистыми глазами.
3. Полный отчёт → `.obsidian-docs/protocols/0028-money-layers-panel/fidelity-report.md`; человеку —
   вердикты НЕ ПОКРЫТО / СДЕЛАНО ИНАЧЕ. Advisory: мерж не блокирует,
   судит человек.
4. Признано дефектом → fix в ветке фичи с тегом
   `[protocol-0028-money-layers-panel/3.5-m-fix]`; принято как есть →
   фиксация в `review-log.md`.

### 4-m. Knowledge Bank
```bash
/kb-update .obsidian-docs/protocols/0028-money-layers-panel/plan.md .obsidian-docs/protocols/0028-money-layers-panel/log.md
```
...
Затем:
```bash
git add .obsidian-docs/knowledge-bank/ && git commit -m "docs(knowledge-bank): update after 0028-money-layers-panel [protocol-0028-money-layers-panel/4-m]"
git push origin 0028-money-layers-panel
```
...
```bash
git add -A && git commit -m "docs: update after 0028-money-layers-panel [protocol-0028-money-layers-panel/4.5-m]"
git push origin 0028-money-layers-panel
```
...
git merge --no-ff 0028-money-layers-panel
...
1. Убедись что все коммиты запушены: `git push origin 0028-money-layers-panel`
...
> `Для завершения выполни /protocol-cleanup 0028-money-layers-panel`
