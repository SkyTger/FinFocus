...
**Багфикс протокола 0029** — `_calculate_recurring_before_date` (база
остатка ДО начала периода) и `_get_recurring_daily_changes` (раскладка
внутри периода) теперь фильтруют recurring-инстансы по ИХ ФАКТИЧЕСКОЙ
дате, а не по `original_date` шаблона:
- `_calculate_recurring_before_date` вычитает savings-типы
  (`SAVINGS_RESERVE`, `SAVINGS_CONTRIBUTION`) как расход — симметрично
  соседям (`_get_recurring_daily_changes`,
  `_get_recurring_totals_for_period`). Раньше учитывались только
  income/expense, из-за чего база остатка завышалась
- Обе выборки расширены за пределы своего периода: `_calculate_recurring_before_date`
  смотрит вправо на `RECURRING_LOOKAHEAD_DAYS` (= `RecurringService.MAX_FORECAST_DAYS`,
  366, привязано импортом — без цикла, `recurring_service` не импортирует
  `calendar_service`), `_get_recurring_daily_changes` — влево до самого
  раннего шаблона. Без расширения exception, перенесённый через границу
  окна расчёта (в любую сторону, для ЛЮБОГО типа операции), терялся из
  баланса целиком
- Остаточное ограничение (в докстринге кода): exception с `original_date`
  дальше 366 дней от границы расчёта не подхватывается — исправление
  требует правки выборки в `RecurringService` (по двум датам), вне
  scope протокола 0029
- `_get_recurring_totals_for_period` (сводки месяца/года на дашборде)
  от переносов через границы периода всё ещё страдает — это отображение
  сводки, не расчёт баланса; кандидат в пул заплаток (не запротоколирован
  отдельно)
- Этим фиксом снято ограничение MoneyLayersService, описанное ниже
  (см. «Формула резерва»)
...
**Известное допущение** (см. докстринг для деталей):
...
**Ограничение СНЯТО протоколом 0029**: раньше перенос savings-exception
с даты ДО `reference_date` на дату внутри окна завышал «Свободно» —
`_calculate_recurring_before_date` игнорировал savings-типы. Багфикс
описан в `services.md` → CalendarService выше. Докстринги
`_goals_part_by_day` и `_collect_operations` актуализированы.
