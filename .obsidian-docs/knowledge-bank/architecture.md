---
name: architecture
description: Архитектура FinFocus — слои Dash-приложения, сервисы, ORM, планируемый редизайн дашборда «щиток»
type: reference
originSessionId: -
---
...
**Следующий крупный шаг: редизайн дашборда «щиток»** (принят
2026-08-23, **реализация не начата**, кода нет):
- Источник решений: `../discussions/_archive/dashboard-as-menu-2026-08-23/design.md`
- Дашборд становится навигацией: боковое меню на главном экране
  убирается, карточки-фрагменты разделов — единственная точка входа
  (клик по карточке = переход в раздел с сохранением контекста, напр.
  клик по «завтра» в мини-календаре открывает календарь на завтра)
- Шапка-вердикт «Свободно сегодня: N ₽» + цветовой статус
- Текущий график cashflow (Батч 5.2, bar+линия) **заменяется** графиком
  «деньги с именами» — декомпозиция прогнозного остатка в полосы
  Свободно/Платежи/Резерв целей и подушки, с вехами целей на оси времени
- Требуется **новая модель данных «свободно/платежи/резерв» по дням** в
  сервисном слое — её сейчас нет (есть только доходы/расходы/баланс);
  единая модель нужна всем карточкам щитка, чтобы устранить
  расхождение цифр между ними
- На остальных экранах текущий Sidebar (`app/components/sidebar.py`)
  заменяется компактной полоской иконок — затрагивает каркас навигации
  всех страниц, не только дашборд
- Секвенирование: до следующего бета-цикла (см. Epic-09 фаза 4 выше)
