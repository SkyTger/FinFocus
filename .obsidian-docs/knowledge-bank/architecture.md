---
name: architecture
description: Архитектура FinFocus — слои Dash-приложения, сервисы, ORM, редизайн дашборда «щиток» (кусок 1 на ревью, куски 2-3 не начаты)
type: reference
originSessionId: -
---
...
**Редизайн дашборда «щиток» (Epic-11)** — 3 куска, кусок 1 РЕАЛИЗОВАН
(протокол 0028, на ревью, PR #28, мерж предстоит):
- Источник решений: `../discussions/_archive/dashboard-as-menu-2026-08-23/design.md`

**~~Кусок 1~~ Модель данных + шапка + график полос**: 🔶 РЕАЛИЗОВАН, на ревью
...
- Контракт `app/schema/money_layers.py` спроектирован ПОД КУСОК 1;
  стабильность до куска 2 не гарантируется — осознанное решение
- Известное ограничение: перенесённый savings-exception внутри месяца
  завышает «Свободно» (латентный дефект в другом сервисе, править
  запрещено ограничением протокола) — кандидат в отдельный протокол
  до начала куска 2

**Кусок 2** (не начат): карточки-двери — единственная точка входа в
разделы на главном экране (клик = переход с сохранением контекста,
напр. клик по «завтра» в мини-календаре открывает календарь на
завтра), пустые состояния, стратегия загрузки, механики-сироты
текущего дашборда (сверка, онбординг-тост)
