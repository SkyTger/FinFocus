...
- **nav_rail.py** - навигация: полоска-меню 60px; чистая функция без
  БД и колбэков, рендерится слотом в `main.py` (протокол 0031;
  снята с дашборда куском 2, сужена куском 3)
- **sidebar.py** - файл-надгробие: одна константа `ADDITIONAL_NAV_ITEMS`
  под будущие маршруты `/settings` и `/help` (решение владельца Р2)
...
## Формальные контракты (реестр не заведён — осознанно)

FinFocus не имеет внешнего API (single-user desktop-style приложение,
Dash callback-и — не публичный контракт) и не использует OpenAPI/
Swagger/GraphQL/proto схемы. Alembic присутствует только как транзитивная
зависимость в `.venv`, в проекте не используется (см. `tech-stack.md`).

Схема БД живёт в идемпотентных миграциях `app/core/migrations.py`
(миграции 001-007, применяются автоматически при старте через
`run_all_migrations()`), а не в отдельном формальном описании —
источник правды по схеме: сами ORM-модели `app/models/database.py`
+ файл миграций. `contracts.md` в этом проекте не заводится: заводить
его при отсутствии формальных описаний — не задача KB (см. правило
Keeper'а про реестр контрактов). Если в проекте появится OpenAPI-
спецификация, схема в отдельном формате или proto — завести
`contracts.md` тогда же.
...
└───────────────────┬─────────────────────────────────────┘
      ...
      ▼             ▼             ▼             ▼             ▼
┌──────────┐ ┌──────────┐ ┌──────────────┐ ┌─────────┐ ┌─────────┐
│dashboard │ │calendar  │ │transactions  │ │ goals   │ │nav_rail │
...
- ✅ Sidebar снят с дашборда, переведён на ноль колбэков (Подход B —
  ...
  в `main.py`; куском 3 переименованы в `create_nav_rail` /
  `render_nav_rail_slot`); механики-сироты (сверка, онбординг-тост) —
  не тронуты (вне scope куска 2)
...
**Кусок 3** (смержен протоколом 0031, 2026-08-30, f504fa8): Sidebar 228px
заменён полоской иконок 60px (`app/components/nav_rail.py`) на всех
экранах, кроме дашборда. Содержимому вернулись 228px ширины.
Закрыты P1 UX-аудита (пункт «Настройки» вёл на 404 — убран до
появления маршрута) и P3 (зашитая «v1.0.0» — версия переехала в окно
профиля из нового `app/version.py`). Разворот полоски при входе с
дашборда — CSS-анимация на монтировании узла, разбор в
`patterns/callbacks.md`. Прежний `sidebar.py` усох до надгробия
с константой `ADDITIONAL_NAV_ITEMS` (решение владельца Р2)
