---
name: protocols
description: История протоколов разработки FinFocus (0002-0032) — контекст, решения, реализация каждого; включая незапротоколированную packaging-работу
type: reference
originSessionId: -
---
...
### Протокол 0032: System Ops Guard (2026-08-30)
**Статус**: ✅ Реализован (ветка `0032-system-ops-guard`, финализация
и ревью пройдены; статус мержа сверять с ROADMAP/feature_progress —
не обновлялись этим прогоном KB)
**Батч**: план «сшивка разделов со щитком» (ROADMAP), шаг 2 из 5 —
защитная заплатка, вне продуктового концепт-обсуждения (шаг 3)

**Контекст**: список операций (`/transactions`) выбирает ВСЕ шесть
типов транзакций, но рендер до протокола различал только
INCOME/EXPENSE — служебные `SAVINGS_RESERVE`/`SAVINGS_CONTRIBUTION`
показывались как обычный «Расход» с активными кнопками
редактирования/удаления, доступны для bulk-выделения и просили
категорию через chips. Календарь эту проблему уже решал (readonly,
эмодзи, «(авто)», `calendar.py:437-500`) — список приведён к тому же
принципу. Класс «защита данных» (прямое удаление минует каскад
`GoalService`), не косметика.

**Решение**: двухуровневая защита. (1) UI не рендерит контролы у
служебных строк (без чекбокса/кнопок/chips, замок с title,
«(авто)» в описании, приглушённый класс `.tx-system-row`); ADJUSTMENT
получил бейдж «Корректировка», TRANSFER — «Перевод» (общий словарь
`TYPE_LABELS`, переиспользован из CSV-экспорта, не задублирован).
(2) Серверные guard'ы: `open_edit_modal`/`handle_delete_click` в
`transaction_modals.py` проверяют предикат `_is_system_transaction`
сразу после загрузки транзакции; `bulk_update_category` исключает
некатегоризируемые типы на уровне сервиса; `_drop_system_ids`
страхует bulk-выборку отдельным запросом к БД; chips-callbacks
проверяют предикат перед записью.

**Единственный источник правды «служебности»**: `_is_system_transaction`
+ `SYSTEM_TRANSACTION_TYPES` в `transactions.py` — импортируется в
`transaction_modals.py`, второго независимого предиката нет.

**Критичный порядок проверок**: в `open_edit_modal` guard «служебная
ли операция» стоит ДО ветвления на recurring/обычная — шаблон
резервирования бюджета реально бывает recurring-операцией, и
обратный порядок пропустил бы служебную запись в диалог выбора scope
раньше проверки. Подробности и общее правило — `patterns/callbacks.md`.

**Решение Р1**: dropdown типов edit-модала умеет только
INCOME/EXPENSE — у ADJUSTMENT скрыта кнопка редактирования, delete
оставлен (откат корректировки сверки — право пользователя). TRANSFER
той же болезнью страдает, но по плану не тронут (вне scope,
пре-существующее ограничение модала).

**Принятый техдолг** (зафиксирован на ревью, не блокер): два
независимых списка типов — `SYSTEM_TRANSACTION_TYPES` (UI-слой, «что
нельзя трогать») и `CATEGORIZABLE_TRANSACTION_TYPES` (сервисный слой,
«что можно категоризировать», инвертированный список) — могут молча
разойтись при добавлении нового `TransactionType`. Автоматической
сверки между ними нет.

**Тесты**: 898 (было 845, +53). `tests/test_transactions_system_ops.py`
(51, без БД где возможно — Transaction в памяти, относительные даты) +
2 в `test_transaction_service.py` (смешанный bulk-список; список из
одних служебных → счётчик 0 без исключения). Mutation smoke: предикат
`_is_system_transaction` → `return False` — 14 падений; delete-guard →
`if False` — 2 падения; оба отката — зелено. flake8 без новых
замечаний (единственное — сдвинутая pre-existing E501-строка).

**Детали реализации**: `modules/ui-components.md` → «Служебные
...
**Статус**: ✅ СМЕРЖЕНО (2026-08-30, `f504fa8`, PR #31)
...
**Тесты**: 825 (было 766). `tests/test_nav_rail.py` (60) — новый
...
**Найдено на ревью и исправлено** (fidelity-гейт, шаг 3.5-m): тесты
входа в профиль проверяли аватар только на одном разделе
(`/calendar`), тогда как AC-7 спеки требует «то же самое проверяется
на каждом из разделов, а не на одном» — и соседние тесты того же
файла уже были параметризованы по `SECTION_PATHS`. Параметризованы,
порча (удаление аватара на одном разделе) теперь ловится; было 43
теста в файле, стало 60.

**Долг куска 3** (зафиксирован ревью, не блокер мержа):
- визуальный слой полоски (разворот, язычки, геометрия) не покрыт
  тестами — **тот же класс долга, что в куске 1 привёл к отдельному
  протоколу 0029** (47 тестов визуального слоя постфактум);
- NFR-3 без замера производительности (в куске 2 замер делали,
  ~20 мс) — риск низкий, полоска строит меньше узлов, чем сайдбар;
- `aria-current` для активного раздела отсутствует (следствие
  ограничения `dcc.Link`) — вопрос доступности вне scope протокола;
- устаревшие упоминания сайдбара в комментариях `main.py:139`,
  `profile_modal.py:139`, `tests/test_profile_modal_callbacks.py`.
