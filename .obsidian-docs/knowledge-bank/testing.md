---
name: testing
description: Стратегия тестирования FinFocus — 693 теста, стратегия относительных дат против календарного протухания, CI на pytest
type: reference
---
...
**693 теста, все проходят** (протокол 0029, на ревью — `693 passed`
= 639 на main + 7 регрессионных для багфикса `CalendarService` +
47 нового `tests/test_dashboard_panel_ui.py`, покрывающего визуальный
слой дашборда-щитка; до 0028 было 565 базовых, проверено 2026-08-21,
Python 3.10.12). Тестов в `tests/`: 39 файлов (`test_*.py`, без
подкаталогов), включая новый `test_dashboard_panel_ui.py`.
...
**Test Execution Time**: полный прогон 639 тестов — около 7.5 секунд
...
**Последнее обновление**: 2026-08-25 (протокол 0029, на ревью — счётчик
639 → 693 теста: +7 регрессионных для багфикса `CalendarService`, +47
для визуального слоя дашборда-щитка, новый файл `test_dashboard_panel_ui.py`).
Предыдущее: 2026-08-25 (протокол 0028, смержено — счётчик 565 → 639
тестов, добавлена практика mutation-проверки формул). До этого:
