---
name: testing
description: Стратегия тестирования FinFocus — 808 тестов, стратегия относительных дат против календарного протухания, CI на pytest
type: reference
---
...
**808 тестов, все проходят** (протокол 0031, PR #31 на ревью —
`808 passed`, 0 xfailed = 766 на main + 50 новых (43
...
**Test Execution Time**: полный прогон 808 тестов — около 7 секунд
...
**Последнее обновление**: 2026-08-30 (протокол 0031, PR #31 на ревью —
счётчик 766 → 808 тестов: +43 `test_nav_rail.py` (полоска-меню;
заменил удалённый `test_sidebar.py` как регрессионный якорь
навигации), +7 `test_version.py` (источник версии, сверка с
git-тегом), +4 в `test_profile_modal_callbacks.py` (версия в окне
профиля), −8 удалённых вместе с сайдбаром, +1 активированный из
xfail. Попутно починен протухавший в конце месяца
