...
не принято. Выпущен релиз `v0.9.0-beta.1`. 565 unit-тестов, все зелёные
(2026-08-21).

**Принято 2026-08-23, реализация не начата**: редизайн дашборда «щиток» —
дашборд становится навигацией (шапка-вердикт «Свободно сегодня», график
полос свободно/платежи/резерв вместо текущего bar+line графика,
карточки-двери в разделы, компактная полоска меню вместо сайдбара на
остальных экранах). Секвенирование: редизайн — до следующего бета-цикла
(фаза 4 Epic-09, сбор фидбека, откладывается до его завершения). Источник
решений: `../discussions/_archive/dashboard-as-menu-2026-08-23/design.md`.
См. также `architecture.md` → «Планируемые изменения».

**Последнее обновление**: 2026-08-23 (после протоколов 0025-0027 + принятия
концепции редизайна дашборда «щиток»)
...
- [protocols.md] - История протоколов разработки (0002-0027) с деталями реализации, включая packaging-врезку и открытый вопрос №1 (способ доставки)
...
2026-08-23, реализация не начата): подробности в `architecture.md` →
«Планируемые изменения» и в
...
**Версия Memory Bank**: 4.2
**Дата создания**: 2026-01-17
**Последнее обновление**: 2026-08-23 (закрытие долга актуализации: протоколы
0025-0027, PyInstaller packaging, релиз v0.9.0-beta.1, принятие концепции
редизайна дашборда «щиток»; 565 тестов)
