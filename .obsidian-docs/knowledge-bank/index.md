...
не принято. Выпущен релиз `v0.9.0-beta.1`. 898 unit-тестов в main (было 825
после протокола 0031 и закрытия тестового долга куска 3 — см. «Epic-11» ниже; +53 протоколом 0032).

**Epic-11 «щиток» ЗАКРЫТ ЦЕЛИКОМ** (все три куска смержены, включая
тестовый долг куска 3; протоколы 0028-0031):
дашборд переделан на модель «свободно/платежи/резерв» — новый
read-only сервис `MoneyLayersService` раскладывает прогнозный остаток
каждого дня на три слоя единой формулой; UI дашборда заменён целиком —
шапка «Свободно сегодня» + график полос вместо прежнего ряда
...
щитка, ранее проверявшегося только вручную. Кусок 2 (карточки-двери)
реализован протоколом 0030: пять карточек-дверей под графиком заменяют
прежнюю раскладку 8/4 (split-таблицы, readonly-подушка, wishlist-виджет
удалены), новый `DashboardPanelService` (один сбор данных за одну
сессию БД вместо трёх), сайдбар снят с дашборда и переведён на ноль
колбэков (Подход B — чистая функция + Store-триггер профиля), переходы
с контекстом (`?focus_date=`, `?goal=`, `?wishlist_item=`) с явным
контрактом владения `url.search` по pathname. Дефект AC-8 (двухуровневая
дверь Wishlist — клик по хотелке всплывал в контейнер и открывал модал
...
228px) смержен протоколом 0031 (2026-08-30, f504fa8): закрыты P1 (пункт
...
**Следующий шаг после Epic-11** — план «сшивка разделов со щитком»
(ROADMAP, секция «План после Epic-11», 5 шагов): шаг 1 (тесты
визуального слоя полоски) закрыт до старта протокола 0032; **шаг 2
(защитная заплатка — служебные операции в списке операций) закрыт
протоколом 0032 (смержено 2026-08-30, 62ac1df)**; шаг 3
(концепт-обсуждение «разделы после щитка») — следующий.

**Протокол 0032 (System Ops Guard, 2026-08-30)** — служебные операции
(`SAVINGS_RESERVE`/`SAVINGS_CONTRIBUTION`) в списке `/transactions`
получили ту же readonly-защиту, что уже была в календаре: UI не
рендерит контролы (чекбокс, edit/delete, chips), серверные guard'ы в
callbacks и фильтрация в сервисном слое (`bulk_update_category`) —
двухуровневая защита. Единственный источник «служебности» —
`_is_system_transaction` в `transactions.py`. Детали: `protocols.md`
(0032), `modules/ui-components.md` → «Служебные операции — маркировка
и защита», `patterns/callbacks.md` → «Порядок guard'ов».

**Последнее обновление**: 2026-08-30 (протокол 0032 — служебные
операции в списке транзакций защищены двухуровневым guard'ом;
825 → 898 тестов)
...
- [protocols.md] - История протоколов разработки (0002-0032) с деталями реализации; Epic-11 «щиток» (0028-0031) закрыт целиком, 0032 — защита служебных операций в списке транзакций; включая packaging-врезку и открытый вопрос №1 (способ доставки)

### Модули системы (краткие описания)
- [modules/database.md] - SQLAlchemy ORM: User (+ avatar_id), Transaction, Goal, GoalContribution
- [modules/services.md] - TransactionService, GoalService, CalendarService, RecurringService, DashboardService, AllocationService, OnboardingService, MoneyLayersService
...
- [modules/schema.md] - TypedDicts для типизации (app/schema/), UserProfile, OnboardingStatus, MoneyLayersData
...
**Epic-11 «щиток» — редизайн дашборда, ЗАКРЫТ ЦЕЛИКОМ (все три куска + тестовый долг куска 3)**:
- [x] ✅ Кусок 1 (протокол 0028, PR #28, смержено 2026-08-25): модель
...
- [x] ✅ Долги куска 1 (протокол 0029, смержено 2026-08-25, 0b3cec5):
  багфикс `CalendarService` (savings-операции в базе остатка) + 47
  тестов визуального слоя щитка
- [x] ✅ Кусок 2 (протокол 0030, PR #30, смержено 2026-08-26, 33c8a11): пять карточек-дверей
  (`app/components/panel_cards.py`) заменяют раскладку 8/4 (split-таблицы,
  readonly-подушка, wishlist-виджет удалены); `DashboardPanelService` —
  один сбор `PanelData` за одну сессию БД; сайдбар снят с дашборда, ноль
  колбэков (Подход B); переходы с контекстом
...
- [x] ✅ Кусок 3: полоска-меню 60px вместо сайдбара 228px (протокол
  0031, смержено 2026-08-30, f504fa8) — `app/components/nav_rail.py`, разворот при
  входе с дашборда, версия в окне профиля из `app/version.py`,
  прежний `sidebar.py` усох до надгробия
- [x] ✅ Тестовый долг куска 3 (ветка test/0031-nav-rail-visual-debt,
  2026-08-30): 20 якорных CSS-тестов визуального слоя полоски
  (`tests/test_nav_rail_visual.py`) — 845 тестов
...
**План «сшивка разделов со щитком» (ROADMAP, после закрытия Epic-11)**,
шаг 2 из 5 выполнен:
- [x] ✅ Шаг 1: тесты визуального слоя полоски (см. выше)
- [x] ✅ Шаг 2 — Протокол 0032 (System Ops Guard, 2026-08-30): служебные
  операции (`SAVINGS_RESERVE`/`SAVINGS_CONTRIBUTION`) в списке
  `/transactions` защищены двухуровневым guard'ом (UI + callbacks +
  сервисный слой) по образцу календаря — 898 тестов. Детали:
  `protocols.md` (0032), `modules/ui-components.md` → «Служебные
  операции — маркировка и защита», `patterns/callbacks.md` → «Порядок
  guard'ов: бизнес-проверка ДО ветвления на подтипы»
- [ ] Шаг 3: концепт-обсуждение «разделы после щитка» (design.md,
  раскрой на эпики по разделам)
- [ ] Шаг 4: переосмысление Epic-10 под эталон `panel.css`
- [ ] Шаг 5: планка беты (фаза 4 Epic-09)
...
├── protocols.md          # История протоколов 0002-0030
...
**Версия Memory Bank**: 4.7
**Дата создания**: 2026-01-17
**Последнее обновление**: 2026-08-30 (протокол 0032 — служебные
операции в списке транзакций защищены двухуровневым guard'ом; план
«сшивка разделов со щитком» шаг 2 выполнен; 825 → 898 тестов)
